...
static ofstream filef("E://dota00.txt");

hnOutDiseaseDxfProvinceRoad::hnOutDiseaseDxfProvinceRoad(void)
{
	// 单位长度
	m_nPixelWidth = 1;
...
bool hnOutDiseaseDxfProvinceRoad::outDiseaseDxf(const char* outPath, hnGridDiseaseInfo gridDisease)
{
	// Test
...
// 设置中心线类型
void hnOutDiseaseDxfProvinceRoad::setCenterType(int nCenterType, int nLineType)
{
...
// 初始化CAD
void hnOutDiseaseDxfProvinceRoad::initCAD(DL_Dxf* dxf, DL_WriterA* dw)
{
	if (dw == NULL)
...
// 初始化绘制图例模块
void hnOutDiseaseDxfProvinceRoad::initDrawSymbolBlock(DL_Dxf* dxf, DL_WriterA* dw)
{
...
// 初始化图例块模型
void hnOutDiseaseDxfProvinceRoad::initSymbolBlock(DL_Dxf* dxf, DL_WriterA* dw)
{
	double dXScale = 0.0;
	double dYScale = 0.0;
...
// 初始化框架模块
void hnOutDiseaseDxfProvinceRoad::initMainFrameBlock(DL_Dxf* dxf, DL_WriterA* dw)
{
...
// 初始化道路模块
void hnOutDiseaseDxfProvinceRoad::initRoadBlock(DL_Dxf* dxf, DL_WriterA* dw)
{
	// 车道实现
	dw->sectionBlocks();
...
// 绘制主框架
void hnOutDiseaseDxfProvinceRoad::drawMainFrame(DL_Dxf* dxf, DL_WriterA* dw, double dx, double dy,
...
// 初始化说明模块
void hnOutDiseaseDxfProvinceRoad::initRemarks(DL_Dxf* dxf, DL_WriterA* dw)
{
	// 备注
	dw->sectionBlocks();
	...
	dxf->writeText(*dw, DL_TextData(0, 3.0*m_nPixelWidth, 0, 0, 0, 0, 0.9*m_nPixelWidth, 1, 0, 0, 0, "注:", "Standard", 0), DL_Attributes("MainFrame", 256, -1, "BYLAYER", 1.0));
	dxf->writeText(*dw, DL_TextData(0, 1.5*m_nPixelWidth, 0, 0, 0, 0, 0.9*m_nPixelWidth, 1, 0, 0, 0, "1、图中的字母L、M、H分别表示病", "Standard", 0), DL_Attributes("Black", 256, -1, "BYLAYER", 1.0));
	dxf->writeText(*dw, DL_TextData(0, 0.0, 0, 0, 0, 0, 0.9*m_nPixelWidth, 1, 0, 0, 0, "害程度的轻型、中型、重型。", "Standard", 0), DL_Attributes("MainFrame", 256, -1, "BYLAYER", 1.0));
...
// 绘制图例
void hnOutDiseaseDxfProvinceRoad::drawSymbol(DL_Dxf* dxf, DL_WriterA* dw, double dx, double dy,
	double dScaleX, double dScaleY)
{
	dw->sectionEntities();
...
// 绘制里程以及车道表示
void hnOutDiseaseDxfProvinceRoad::drawRoad(DL_Dxf* dxf, DL_WriterA* dw, double dx, double dy,
	double dScaleX, double dScaleY)
{
...
// 绘制病害
void hnOutDiseaseDxfProvinceRoad::drawDisease(const char* strBlock,
	hnDiseaseInfoBase* diseaseInfo, DL_Dxf* dxf, DL_WriterA* dw)
{
		...
		|| diseaseInfo->getDiseaseType() == SHUINI_ROAD_TYPE && diseaseInfo->nDiseaseType == hnShuiNiDiseaseInfo::LF_SN_DISEASE_TYPE)
	{
		...
		if((diseaseInfo->getDiseaseType() ==LIQING_ROAD_TYPE &&diseaseInfo->nDiseaseType == hnLiqingDiseaseInfo::LF_HX_DISEASE_TYPE) 
			||  (diseaseInfo->getDiseaseType() ==SHUINI_ROAD_TYPE &&diseaseInfo->nDiseaseType == hnShuiNiDiseaseInfo::LF_HX_DISEASE_TYPE))//纵向
		{
		...
		|| diseaseInfo->getDiseaseType() == SHUINI_ROAD_TYPE && diseaseInfo->nDiseaseType == hnShuiNiDiseaseInfo::LF_SN_YZ_DISEASE_TYPE)
	{
		...
		double dLBX,  dLBY,  dWidth,  dHeight;
		getBoundary(diseaseInfo,dLBX,dLBY,dWidth,dHeight);
		if((diseaseInfo->getDiseaseType() ==LIQING_ROAD_TYPE &&diseaseInfo->nDiseaseType == hnLiqingDiseaseInfo::LF_HX_YZ_DISEASE_TYPE) 
			||(diseaseInfo->getDiseaseType() ==SHUINI_ROAD_TYPE &&diseaseInfo->nDiseaseType == hnShuiNiDiseaseInfo::LF_HX_YZ_DISEASE_TYPE) )//纵向
		{
			vecPtNew.push_back(hn2dPt(dLBX+dWidth/2,dLBY));
			vecPtNew.push_back(hn2dPt(dLBX+dWidth/2,dLBY+dHeight));
		}
		...
		|| diseaseInfo->getDiseaseType() ==SHUINI_ROAD_TYPE &&diseaseInfo->nDiseaseType == hnShuiNiDiseaseInfo::TZXB_SN_DISEASE_TYPE)
	{
		...
		for (int i = 0; i < vecPtNew.size() - 1; i++)
		{
			dDist = sqrt((vecPtNew[i+1].dx - vecPtNew[i].dx)*(vecPtNew[i+1].dx - vecPtNew[i].dx) +
				(vecPtNew[i+1].dy - vecPtNew[i].dy)*(vecPtNew[i+1].dy - vecPtNew[i].dy));

			ptNormal.dx = (vecPtNew[i+1].dx - vecPtNew[i].dx) / dDist;
			ptNormal.dy = (vecPtNew[i+1].dy - vecPtNew[i].dy) / dDist;

			ptFir.dx = vecPtNew[i].dx + ptNormal.dx * 0.2*dDist;
			ptFir.dy = vecPtNew[i].dy + ptNormal.dy * 0.2*dDist;

			ptSec.dx = vecPtNew[i].dx + ptNormal.dx * 0.8*dDist;
			ptSec.dy = vecPtNew[i].dy + ptNormal.dy * 0.8*dDist;

			dxf->writeLine(*dw, DL_LineData(vecPtNew[i].dx, vecPtNew[i].dy, 0.0, ptFir.dx, ptFir.dy, 0.0), DL_Attributes("MainFrame", 256, -1.0, "BYLAYER", 8.0));//2
			dxf->writeLine(*dw, DL_LineData(ptFir.dx, ptFir.dy, 0.0, ptSec.dx, ptSec.dy, 0.0), DL_Attributes("Test6", 256, -1.0, "BYLAYER", 8.0));//2
			dxf->writeLine(*dw, DL_LineData(ptSec.dx, ptSec.dy, 0.0, vecPtNew[i+1].dx, vecPtNew[i+1].dy, 0.0), DL_Attributes("MainFrame", 256, -1.0, "BYLAYER", 8.0));//2
...
// 绘制备注
void hnOutDiseaseDxfProvinceRoad::drawRemark(string strBegMileage, string strEndMileage, int nLineType,
...
// 坐标转换--usertoscreen
void hnOutDiseaseDxfProvinceRoad::convertCoord(double dx, double dy, double& outX, double& outY)
{
	double dTemp = dx;
...
// 获取包围盒
void hnOutDiseaseDxfProvinceRoad::getBoundary( hnDiseaseInfoBase* diseaseInfo, double& dLBX, double& dLBY, double& dWidth, double& dHeight)
{
...
// 获取病害外边框的参数信息
void hnOutDiseaseDxfProvinceRoad::getRectInfo(vector<hn2dPt>& vecPt, hn2dPt& ptOri, 
