...
void DL_Dxf::addPolyline(DL_CreationInterface* creationInterface) {
	...
	if (maxVertices < vertexIndex+ 1)
	{
		maxVertices = vertexIndex;
	}
	else
	{
		maxVertices = vertexIndex + 1;
	}
...
void DL_Dxf::writeHatch2(DL_WriterA& dw,
    ...
    if (data.solid==false) {
        ...
        //dw.dxfInt(78, 0);
        dw.dxfInt(78, 1);
        dw.dxfReal(53, data.angle);
        dw.dxfReal(43, data.originX);
        dw.dxfReal(44, data.originY);
		//2020-12-22修改  图案的间隔由比例调整
		if (data.pattern == "MUDST")
		{
			dw.dxfReal(45, 0.5461000000000001);
			dw.dxfReal(46, 0.27305);
			dw.dxfInt(79, 6);
			dw.dxfReal(49, 0.27305);
			dw.dxfReal(49, -0.27305);
			dw.dxfReal(49, 0.0);
			dw.dxfReal(49, -0.27305);
			dw.dxfReal(49, 0.0);
			dw.dxfReal(49, -0.27305); 
		//	dw.dxfReal(47, 0.0006597637350776);
		}
		else
		{
			dw.dxfReal(45, 0.125*data.scale);
			dw.dxfReal(46, 0.125*data.scale);
			dw.dxfInt(79, 2);
			dw.dxfReal(49, 0.125*data.scale);
			dw.dxfReal(49, -0.125*data.scale);
		}
