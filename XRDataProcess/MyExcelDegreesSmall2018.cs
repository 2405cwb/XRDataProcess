...
{
    /// <summary>
    /// 等级公路规范2018，JTG 5210-2018 公路技术状况评定标准，小方格
    /// </summary>
    class MyExcelDegreeSmall2018
    {
            ...
            bool IsDis, bool IsMeanIRI, bool IsMeanMTD, bool IsMeanRut, bool IsPBI, bool IsSpeed, bool IsMeanMPD = false, bool IsGeoAlig = false)
        {
            ...
            if (IsDis)
            {
                ...
                // GlobalExcel.GetSmallRectAllDis(prjdir.FullName, prjinfo, prjinfo._Direction, _RoadGradeDict, _SRutDisVal, _SRutDisMile, ref _SmallRoadDisList, ref _SmallRoadRepairList, _rutThresh);
                GlobalExcel.GetSmallRectAllDis_Select(prjdir.FullName, prjinfo, prjinfo._Direction, _RoadGradeDict, _SRutDisVal, _SRutDisMile, ref _SmallRoadDisList, ref _SmallRoadRepairList, _rutThresh, _RoadPart);
            }
            ...
            if (IsMeanMPD) MPDRes = GlobalExcel.GetMPDMeanVal(prjinfo, prjdir, _RoadPart, ref _LMPDMeanVal, ref _RMPDMeanVal, ref _CMPDMeanVal, _Setting.IsWarning);

            if (_Setting.ExcelType == 4 || _Setting.ExcelType == 17 || _Setting.ExcelType == 18 || _Setting.ExcelType == 15) GPSRes = GlobalExcel.GetGPSInfo(prjinfo, prjdir, _RoadPart, ref _GPSInfo);

            return IRIRes && RutRes && MTDRes && GPSRes && GeoAligRes;
        }
        ...
        public static void OutputIRI(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            string Destxls = string.Format(@"{0}\{1}_IRI_{2}m.xlsx", path, prjdir.Name, disval);   // string.Format("K{0:000+000}-K{1:000+000}", prjinfo._StartMile, prjinfo._EndMile),
            //string Destxls = string.Format(@"{0}\{1}_{2}_{3}_{4}-{5}_{6}_IRI_{7}m.xlsx", path, prjinfo._RoadCode, prjinfo._RoadName, prjinfo._Direction, prjinfo._StartMile, prjinfo._EndMile, prjinfo._DataDate, disval);
        ...
        {
            disval *= 10;
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\CPMS路面病害调查表-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            WriteZJGTDisHZTJ2Xls(_Worksheet_sndc, _Worksheet_lqdc, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, disval, _MarkVal);
     ...
     DirectoryInfo prjdir, SmalRectDisease[] arrdis, List<MilePart> roadpart, ExcelGPS[] gpsInfos)
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\路面病害面积统计表-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            if (_Setting.ExcelType == 19)
            {
                srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\甘肃\路面病害面积统计表-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            }
            ...
            if (_Setting.ExcelType == 19)
            {
                ...
                WriteDisHZ2Xls(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, ref Haslqflag, ref Hassnflag, 5, 53);
        ...
        public static void outPutAutoTest(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir)
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\等级公路自动化检测.xlsx",
                System.Windows.Forms.Application.StartupPath);
            string Destxls = string.Format(@"{0}\{1}_等级公路自动化检测数据.xlsx", path, prjdir.Name);
            MSExcel.Workbook _Workbook = excelApp.Workbooks.Open(srcxls, Type.Missing,
                true, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing,
                Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            ...
            MSExcel.Worksheet _Worksheet_iri = _Workbook.Sheets["路面平整度自动化检测数据文件"] as MSExcel.Worksheet;
            writeAutoTestXls_iri(_Worksheet_iri, _RoadPart, _LIRIMeanVal, _RIRIMeanVal, _SpeedVal);
            MSExcel.Worksheet _Worksheet_lf = _Workbook.Sheets["路面裂缝自动化检测数据文件"] as MSExcel.Worksheet;
            writeAutoTestXls_lf(_Worksheet_lf, _RoadPart, _SmallRoadDisList, prjinfo);
            MSExcel.Worksheet _Worksheet_gc = _Workbook.Sheets["断面高程自动化检测数据文件"] as MSExcel.Worksheet;
            //高程数据用错了，应该用平整度得 ，这里用的车辙的 
            // writeAutoTestXls_gc(_Worksheet_gc,_RoadPart, prjdir,prjinfo,_SpeedVal);
            MSExcel.Worksheet _Worksheet_cz = _Workbook.Sheets["路面车辙自动化检测数据文件"] as MSExcel.Worksheet;
            writeAutoTestXls_cz(_Worksheet_cz, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal, _RoadPart);
            MSExcel.Worksheet _Worksheet_jhzk = _Workbook.Sheets["路面几何状况自动化检测数据文件"] as MSExcel.Worksheet;
        ...
        /// n1为右边界第一个点
        /// </summary>
        /// <param name="chezhe"></param>
        /// <param name="n"></param>
        /// <param name="n1"></param>
        static void digui(int n, ref List<int> chezhe)
        {
            if (n > _RoadConfig.PartWidthNum * _RoadConfig.PartHeightNum)
            {
                return;
            }
            int aver = _RoadConfig.PartWidthNum * _RoadConfig.PartHeightNum / 2;
            int min = 0; //aver-14
            for (min = n; min < n + 8; min++)
            {
                chezhe.Add(min);
            }
            for (min = n + 20; min < n + 28; min++)
            {
                chezhe.Add(min);
            }
            digui(n + _RoadConfig.PartWidthNum, ref chezhe);
        }
        ...
        /// <param name="?"></param>
        /// <param name="?"></param>
        private static void writeAutoTestXls_lf(MSExcel.Worksheet _Worksheet, List<MilePart> roadpart10, SmalRectDisease[] arrdis, ProjectInfo prjinfo)
        {

            int len = roadpart10.Count - 1, dlen = arrdis.Length;
            object[,] disvalsn = new object[len, 4];
            int picAllArea = _RoadConfig.PartWidthNum * _RoadConfig.PartHeightNum / 100;
            MSExcel.Range destrange;
            //根据2014公路路面技术状况自动化检测得到的车辙范围  0.6~1.4m
            List<int> chezheArea = new List<int>();

            digui(_RoadConfig.PartWidthNum / 2 - 14, ref chezheArea);
            int typeidx = 0;
            bool res = false;
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                RoadDiseaseTypes.Clear();
                double lfRatio = 0;
                double hxlfRatio = 0;//横向裂缝率
                double ljdRatio = 0; //轮迹带裂缝率
                while (j < dlen && ((prjinfo._Direction > 0 && arrdis[j].m_mile >= smile && arrdis[j].m_mile < emile)
                    || (prjinfo._Direction < 0 && arrdis[j].m_mile <= smile && arrdis[j].m_mile > emile)))
                {
                    res = RoadDiseaseTypes.DiseaseTypeDict[roadpart10[i].roadtype].TryGetValue(string.Format("{0}.{1}",
                            arrdis[j].RoadType, arrdis[j].RoadDisType), out typeidx);

                    if (res)
                    {
                        double bhArea = arrdis[j].Area;
                        RoadDiseaseTypes.roaddis[roadpart10[i].roadtype][typeidx].totalarea += bhArea;
                        int yesLfnumber = 0; //在范围内的裂缝个数
                        if (arrdis[j].RoadDisType.Contains("裂缝"))
                        {


                            lfRatio += lfRatio + bhArea / picAllArea;
                            //裂缝比例

                            string[] temp = arrdis[j].dispos.Split('-');

                            var temp1 = from p in temp
                                        where p != ""
                                        select Convert.ToInt32(p);
                            foreach (int item in temp1)
                            {
                                if (chezheArea.Contains(item))
                                {
                                    yesLfnumber++;
                                }
                            }
                        }
                        if (arrdis[j].RoadDisType.Contains("横向裂缝"))
                        {
                            hxlfRatio += hxlfRatio + bhArea / picAllArea;
                        }
                        // GlobalExcel.getArea
                        double count = chezheArea.Count;
                        ljdRatio += ljdRatio + yesLfnumber / count;
                    }
                    else
                    {
                        string errval = string.Format("拉框病害信息：{0} 路面材质：{1}\r\n", arrdis[j].GetDisInfoStr(), GlobalExcel._RoadTypeStr[roadpart10[i].roadtype]);
                        //File.AppendAllText(errlog, errval, Encoding.UTF8);
                    }
                    ++j;
                }
                disvalsn[i, colcnt++] = smile * 0.001;
                disvalsn[i, colcnt++] = lfRatio;
                disvalsn[i, colcnt++] = ljdRatio;
                disvalsn[i, colcnt++] = hxlfRatio;
            }
            destrange = _Worksheet.get_Range(String.Format("A2:D{0}", len + 1));
        ...
        /// <param name="?"></param>
        /// <param name="?"></param>
        private static void writeAutoTestXls_gc(MSExcel.Worksheet _Worksheet, List<MilePart> roadpart10, DirectoryInfo path, ProjectInfo proj, double[] speedVal)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    int emile = roadpart10[i + 1].mile;
                    disvalsn[i, colcnt++] = smile * 0.001;
                    for (int n = 0; n < allData[i].Length && n < 2000; n += 100)
                    {
                        disvalsn[i, colcnt++] = allData[i][n] * 10; //原始高程单位 mm
                    }
        ...
        private static void writeAutoTestXls_cz(MSExcel.Worksheet _Worksheet, double[] LRutMeanVal, double[] RRutMeanVal, double[] SRutMeanVal, List<MilePart> roadpart10)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                int emile = roadpart10[i + 1].mile;
                disvalsn[i, colcnt++] = smile * 0.001;
                disvalsn[i, colcnt++] = LRutMeanVal[i];
                disvalsn[i, colcnt++] = RRutMeanVal[i];
                disvalsn[i, colcnt++] = SRutMeanVal[i];
        ...
        /// </summary>
        /// <param name="_Worksheet"></param>
        private static void writeAutoTestXls_iri(MSExcel.Worksheet _Worksheet, List<MilePart> roadpart10, double[] LIRIMeanVal, double[] RIRIMeanVal, double[] SpeedVal)
        {
            int len = roadpart10.Count - 1;
            object[,] disvalsn = new object[len, 4];
            MSExcel.Range destrange;
            for (int i = 0; i < len; i++)
            {
                ...
                int emile = roadpart10[i + 1].mile;
                disvalsn[i, colcnt++] = smile * 0.001;
                disvalsn[i, colcnt++] = LIRIMeanVal[i];
        ...
        private static void WriteDisLB2Xls_roadpart(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir, SmalRectDisease[] arrdis, List<MilePart> roadpart)
        {
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            if (len < 1 || dlen < 1)
                return;
        ...
        /// 
        /// </summary>
        /// <param name="worksheet_snhz"></param>
        /// <param name="worksheet_lqhz"></param>
        /// <param name="prjinfo"></param>
        /// <param name="prjdir"></param>
        /// <param name="roadpart"></param>
        /// <param name="arrdis"></param>
        /// <param name="Haslqflag"></param>
        /// <param name="Hassnflag"></param>
        /// <param name="DataStartXlsxRow"></param>
            ...
            int DataStartXlsxRow, int borderType)
        {
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                if (roadpart[i].roadtype == 1)//水泥
                {
                    ...
                    for (int di = 0, kk = 0; di < disnum; ++di, ++kk)
                    {
                        if (_Setting.ExcelType == 19)
                        {
                            ...
                            {
                                disval[0, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][di].totalarea  / 1;
                    ...
                    disval[0, disnum + 1] = string.Format("=100-{0}*POWER(P{1},{2})",
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        rowcnt_sn_s, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);

                    disval[0, disnum + 2] = string.Format("=IF(Q{0}>={1},\"优\",IF(Q{0}>={2},\"良\",IF(Q{0}>={3},\"中\",IF(Q{0}>={4},\"次\",\"差\"))))",
                        rowcnt_sn_s,
                ...
                else if (roadpart[i].roadtype == 0)//沥青
                {
                    ...
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = prjinfo._RoadNum;  //车道
                    ...
                    for (int di = 0, kk = 0; di < disnum; ++di, ++colcnt, ++kk)
                    {
                        ...
                        if (_Setting.ExcelType==19)
                        {
                            ...
                            {
                                disval[0, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][di].totalarea  / 0.2 ;
                            }
                            else
                                disval[0, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][di].totalarea;
                        }
                    ...
                    drval = ComputPCI_DisArea(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                    disval[0, disnum] = drval;
                    disval[0, disnum + 1] = string.Format("=100-{0}*POWER(Q{1},{2})",
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        rowcnt_lq_s, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);

                    disval[0, disnum + 2] = string.Format("=IF(R{0}>={1},\"优\",IF(R{0}>={2},\"良\",IF(R{0}>={3},\"中\",IF(R{0}>={4},\"次\",\"差\"))))",
                        rowcnt_lq_s,
                        _PCIGrade[roadpart[i].roaddegree][0],
                        _PCIGrade[roadpart[i].roaddegree][1],
                        _PCIGrade[roadpart[i].roaddegree][2],
            ...
            destrange = worksheet_lqhz.get_Range(String.Format("A1:S{0}", rowcnt_lq_s));
            GlobalExcel.SetBorderLine(destrange, borderType);
            destrange = worksheet_snhz.get_Range(String.Format("A1:R{0}", rowcnt_sn_s));
            GlobalExcel.SetBorderLine(destrange, borderType);
            ...
            else
            {
                if (!_Setting.IsOutputDisAreaSubtotal)
                {
                    if (prjinfo._Direction == -1)
                    {
                        ...
                        if (_Setting.IsExcelSort)
                        {
                            MSExcel.Range destrange1 = worksheet_lqhz.get_Range(string.Format("A5:S{0}", rowcnt_lq_s - 1));
            ...
            else
            {
                if (!_Setting.IsOutputDisAreaSubtotal)
                {
                    if (prjinfo._Direction == -1)
                    {
                        ...
                        if (_Setting.IsExcelSort)
                        {
                            MSExcel.Range destrange1 = worksheet_snhz.get_Range(string.Format("A5:R{0}", rowcnt_sn_s - 1));
        ...
        private static double ComputPCI_sichuan(RoadDiseaseType[][] disarea, int roadtype, double partarea)
        {
            double sumarea = 0;
        ...
        /// <param name="partarea"></param>
        /// <returns></returns>
        private static double ComputPCI_AllDisArea(RoadDiseaseType[][] disarea, int roadtype, double partarea)
        {
            double sumarea = 0;
            int len = _RoadSocre[roadtype].Keys.Count;
       ...
       List<MilePart> roadpart, SmalRectDisease[] arrdis)
        ...
        private static void WritePrj2CPMSXls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo)
        {
            ...
            if (prjinfo._Direction > 0)
            {
                _Worksheet.Cells[3, 5] = "上行" + prjinfo._RoadNum;
            }
            else
            {
                _Worksheet.Cells[3, 5] = "下行" + prjinfo._RoadNum;
            }
        ...
        public static void OutputPBI(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            WritePB2Xls(_Worksheet, prjinfo, prjdir, _RoadPart10, _LDeltaHVal, _RDeltaHVal, _SpeedVal10, _MarkVal10, 4);
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\综合报表模板GPS-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            WriteGPSAll2Xls(_Worksheet_PQI, _Worksheet_PCI, _Worksheet_RQI, _Worksheet_RDI,
                _Worksheet_MTD, _Worksheet_DR, _Worksheet_IRI, _Worksheet_Rut, _Worksheet_PBI,
                _Worksheet_PWI, _Worksheet_PB, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
                _LIRIMeanVal, _RIRIMeanVal, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal,
                _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, _PBIVal, _GPSInfo);
            WriteGPSDis2Xls(_Worksheet_lq, _Worksheet_sn, prjinfo, prjdir, _SmallRoadDisList);
        ...
        private static void WriteGPSDis2Xls(MSExcel.Worksheet worksheet_lq, MSExcel.Worksheet worksheet_sn,
            ProjectInfo prjinfo, DirectoryInfo prjdir, SmalRectDisease[] dislist)
        {
            object[,] disinfo = new object[1, 15];
            ...
            for (int i = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                {
                    destrange = worksheet_lq.get_Range(string.Format("A{0}:O{0}", rowcnt_lq));
                    destrange.Value2 = disinfo;
                    rowcnt_lq++;
                }
                else if (dislist[i].RoadType == "水泥")
                {
                    destrange = worksheet_sn.get_Range(string.Format("A{0}:O{0}", rowcnt_sn));
                    destrange.Value2 = disinfo;
                    rowcnt_sn++;
                }
            }
            ...
            {
                destrange = worksheet_lq.get_Range(String.Format("B2:O{0}", rowcnt_lq - 1));
        ...
        public static void OutputZJGTDis(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            {
                string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\中交国通\002-路面损坏报告模板-Small.xlsx",
                    System.Windows.Forms.Application.StartupPath);
                _Workbook = excelApp.Workbooks.Open(srcxls, Type.Missing,
                    true, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
                    Type.Missing, Type.Missing, Type.Missing, Type.Missing,
            ...
            if (disval == 100)
            {
                ...
                WriteZJGTDisHZTJ2Xls(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, disval * 10, _MarkVal);
            }
            else if (disval == 1000)
            {
                ...
                WriteZJGTPCI2Xls(_Worksheet_snpci, _Worksheet_lqpci, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);
            }
            ...
            ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int xlslen, string[] MarkVal)
        {
                ...
                if (emile % xlslen == 0 || (MarkVal[i + 1] != null && MarkVal[i + 1].Contains("路面单元")))
                {
                    if (sn_csmile != sn_cemile)
                    {
                        ...
                        if (emile != roadpart[len].mile)
                        {
                            ...
                            int rownum = srcrange.Rows.Count;
                            MSExcel.Range rang1, rang2;
                            for (int ri = 0; ri < rownum; ri++)
                            {
                                rang1 = worksheet_snhz.get_Range(string.Format("A{0}", sn_tablerow * (tcnt_sn - 1) + 1 + ri));
                                rang2 = worksheet_snhz.get_Range(string.Format("A{0}", sn_tablerow * tcnt_sn + 1 + ri));
                                rang2.RowHeight = rang1.RowHeight;
                            }
                        }
                        sn_flag = false;
                        sn_csmile = sn_cemile;
                    ...
                    if (lq_csmile != lq_cemile)
                    {
                        ...
                        if (emile != roadpart[len].mile)
                        {
                            ...
                            for (int ri = 0; ri < rownum; ri++)
                            {
                                rang1 = worksheet_lqhz.get_Range(string.Format("A{0}", lq_tablerow * (tcnt_lq - 1) + 1 + ri));
                                rang2 = worksheet_lqhz.get_Range(string.Format("A{0}", lq_tablerow * tcnt_lq + 1 + ri));
                                rang2.RowHeight = rang1.RowHeight;
                            }
                        }
                        lq_flag = false;
            ...
            {
                disnum = RoadDiseaseTypes.DiseaseTypeDict[1].Count;
                srcrange = worksheet_snhz.get_Range(String.Format("A{0}:R{1}", sn_tablerow * tcnt_sn + 1, sn_tablerow * (tcnt_sn + 1) + 1));
                destrange = worksheet_snhz.get_Range(String.Format("F{0}:O{1}", sn_tablerow * tcnt_sn + 7, sn_tablerow * tcnt_sn + 6 + disnum));
            ...
            {
                disnum = RoadDiseaseTypes.DiseaseTypeDict[0].Count;
                srcrange = worksheet_lqhz.get_Range(String.Format("A{0}:R{1}", lq_tablerow * tcnt_lq + 1, lq_tablerow * (tcnt_lq + 1) + 1));
                destrange = worksheet_lqhz.get_Range(String.Format("F{0}:O{1}", lq_tablerow * tcnt_lq + 7, lq_tablerow * tcnt_lq + 6 + disnum));
                object[,] ttobj = (object[,])destrange.Value2;
                bool hasdata = false;
                for (int i = 1; i <= disnum; ++i)
        ...
        public static void OutputZJGTPQI(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            WriteZJGTAll2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
        ...
        private static void WriteZJGTAll2Xls(MSExcel.Worksheet worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir,
           List<MilePart> roadpart, SmalRectDisease[] arrdis, double[] LIRIVal, double[] RIRIVal,
           double[] LRutVal, double[] RRutVal, double[] SRutVal, double[] LMTDVal, double[] RMTDVal, double[] CMTDVal)
        {
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                //if (prjinfo._IsDIRIMTD)
                //{
                //    mtdval = Math.Round((LMTDVal[i] + RMTDVal[i]) * 0.5, 2);
                //}
                //else
                //{
                //    mtdval = Math.Round(LMTDVal[i], 2);
                //}
                //mxlist[i, 11] = mtdval;
                #endregion
        ...
        public static void OutputALTDIS(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            {
                srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\奥路通\001-路面病害模板-Small.xlsx",
                System.Windows.Forms.Application.StartupPath, disval);
            ...
            WriteALTDisHZTJ2Xls(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);
            string sheetname = "路面技术状况明细表";
            _Worksheet = _Workbook.Sheets[sheetname] as MSExcel.Worksheet;
            WriteALTPQI2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
            ...
            //    System.Windows.Forms.Application.StartupPath, disval);
            //Destxls = string.Format(@"{0}\{1}-七项指标-{2}m.xlsx", path, prjinfo._RoadCode, disval);
            //_Workbook = excelApp.Workbooks.Open(srcxls, Type.Missing,
            //    true, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
            //    Type.Missing, Type.Missing, Type.Missing, Type.Missing,
            //    Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            ...
            //    MSExcel.XlSaveAsAccessMode.xlNoChange, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            //_Worksheet = _Workbook.Sheets["Sheet1"] as MSExcel.Worksheet;
            //WriteALT_PT_PQI2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
           ...
           ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis)
        {
            ...
            worksheet_snhz.Cells[2, 12] = prjinfo._Direction > 0 ? "上行" : "下行";
            worksheet_lqhz.Cells[2, 12] = prjinfo._Direction > 0 ? "上行" : "下行";
            string errlog = prjdir.FullName + "\\errlog.txt";
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            ...
            double[] LMTDVal, double[] RMTDVal, double[] CMTDVal, int[][] PBVal, int disval)
        {
            ...
            worksheet.Cells[1, 4] = prjinfo._RoadCode;
        ...
        public static void OutputZNRoad(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            WriteZNHZ2Xls2(_Worksheet_hz, _Worksheet_tj, sheetIRI, sheetRUT, sheetDR, sheetPB, sheetPW,
                prjinfo, prjdir, _RoadPart, _SmallRoadDisList, _LIRIMeanVal, _RIRIMeanVal, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal, _PBIVal, _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal);
        ...
        public static void OutputZNDataRoad(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            SmallWritePCI2Xls(_Worksheet_PCI, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, _SpeedVal, _MarkVal, 3);
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\中南安环\病害统计表-Small.xlsx",
                           System.Windows.Forms.Application.StartupPath);
            ...
            WriteZNDis2Xls(_Worksheet_mx, _Worksheet_hz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);
        ...
        private static void WriteZNDis2Xls(MSExcel.Worksheet _Worksheet_mx, MSExcel.Worksheet _Worksheet_hz,
            ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] dislist)
        {
            string disname = null;
            ...
            destrange = _Worksheet_mx.get_Range(String.Format("A3:F{0}", dlen + 2));
            destrange.Value2 = disinfo;
            GlobalExcel.SetBorderLine(destrange, 63);
            //上行 沥青
            int m = 0;
            ...
            for (int n = 0; n < distype.Length; n++)
            {
                ...
                if (n == 19)
                {
                    ...
                    _Worksheet_hz.Cells[m, colidx + 1] = string.Format("=SUMIF(病害列表!D:D,\"{0}\",病害列表!F:F)+", distype[n]) + string.Format("SUMIF(病害列表!D:D,\"{0}\",病害列表!F:F)", distype[n + 1]);
                    n++;
                }
                else
                {
                    if (n > 19)
                    {
                        ...
                        _Worksheet_hz.Cells[m, colidx + 1] = string.Format("=SUMIF(病害列表!D:D,\"{0}\",病害列表!F:F)", distype[n]);
                        if (n == 21)
                        {
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\综合报表模板-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            string Destxls = string.Format(@"{0}\{1}_{2}m.xlsx", path, prjdir.Name, disval);
            MSExcel.Workbook _Workbook = excelApp.Workbooks.Open(srcxls, Type.Missing,
                true, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
        ...
        #endregion
        #region 四川公路院
        private static void WriteDisLB2Xls_roadpart_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir, SmalRectDisease[] arrdis, List<MilePart> roadpart)
        {
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            if (len < 1 || dlen < 1)
                return;
            ...
            int typeidx = 0;
            bool res = false;
            string dirStr = prjinfo._Direction == 1 ? "右幅" : "左幅";
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                int smile = roadpart[i].mile;
                int emile = roadpart[i + 1].mile;
                    ...
                    || (prjinfo._Direction < 0 && arrdis[j].m_mile <= smile && arrdis[j].m_mile > emile)))
                {
                    ...
                    {
                        string[] s = arrdis[j].RoadDisType.Split('.');
                        vallist[rowcnt, 0] = prjinfo._RoadName;

                        DateTime dateTime = DateTime.ParseExact(prjinfo._DataDate, "yyyyMMdd", CultureInfo.InvariantCulture);
                        string formattedString = dateTime.ToString("yyyy/MM/dd"); 
                        vallist[rowcnt, 1] = formattedString;
                        vallist[rowcnt, 2] = arrdis[j].m_mile;
                        vallist[rowcnt, 3] = dirStr;
                         vallist[rowcnt, 4] = prjinfo._RoadNum;
                        vallist[rowcnt, 5] = s[0];
                        if (s.Length > 1)
                        {
                            vallist[rowcnt, 6] = s[1];
                        }
                        else
                        {
                            vallist[rowcnt,6] = "无";
                        ...
                        vallist[rowcnt, 7] = arrdis[j].Area;
                        vallist[rowcnt, 8] = arrdis[j].imgname;
                        vallist[rowcnt, 9] = arrdis[j].imgpath;
            ...
            destrange = _Worksheet.get_Range(String.Format("A3:K{0}", dlen + 2));
            GlobalExcel.SetBorderLine(destrange, 53);
            if (_Setting.Out_roadimg == 0)
            {
                ((MSExcel.Range)_Worksheet.Cells[System.Reflection.Missing.Value, 11]).EntireColumn.Delete();
            ...
            if (_Setting.IsExcelSort)
            {
                MSExcel.Range destrange1 = _Worksheet.get_Range(string.Format("A3:K{0}", dlen + 2));
                MSExcel.Range sortrange = _Worksheet.get_Range(string.Format("C3:C{0}", dlen + 2));
                GlobalExcel.ReflectionColnum(_Worksheet, destrange1, sortrange); 
            }
        }

 

        private static void WritePB2Xls_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir,
         List<MilePart> roadpart, double[] LDeltaHVal, double[] RDeltaHVal, double[] SpeedVal, string[] MarkVal,
         int DataStartXlsxRow)
        {
            string dirStr = prjinfo._Direction == 1 ? "右幅" : "左幅";
            ...
            for (i = 0; i < len; ++i)
            {
                ...
                vallist[i, 4] = dirStr;
                vallist[i, 5] = prjinfo._RoadNum; 

                if (i < LDeltaHVal.Length)
                {
                    vallist[i, 6] = LDeltaHVal[i];
                }
                if (prjinfo._IsDIRIMTD)
                {
                    if (i < RDeltaHVal.Length)
                    {
                        vallist[i, 7] = RDeltaHVal[i];
                    }
                }
                vallist[i, 8] = string.Format("=MAX(G{0},H{0})", i + DataStartXlsxRow);
                vallist[i, 9] = string.Format("=IF(I{0}<{1},\"无跳车\",IF(I{0}<{2},\"轻度跳车\",IF(I{0}<{3},\"中度跳车\",\"重度跳车\")))",
                    i + DataStartXlsxRow, _PBIThresh[0], _PBIThresh[1], _PBIThresh[2]);
                vallist[i,10] = GlobalExcel._RoadTypeStr[roadpart[i].roadtype];
                if (SpeedVal != null)
                {
                    vallist[i, 11] = SpeedVal[i];
                }
                vallist[i,12] = MarkVal[i];
            }
            ...
            destrange = _Worksheet.get_Range(String.Format("A1:K{0}", len + DataStartXlsxRow - 1));
            GlobalExcel.SetBorderLine(destrange, 53);

            if (_Setting.IsExcelSort)
            {
                
                MSExcel.Range destrange1 = _Worksheet.get_Range(string.Format("A4:K{0}", len + DataStartXlsxRow - 1));
                MSExcel.Range sortrange = _Worksheet.get_Range(string.Format("C4:C{0}", len + DataStartXlsxRow - 1));
                GlobalExcel.ReflectionColnum(_Worksheet, destrange1, sortrange);
            }
        }
        private static void WritePBI2Xls_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir,
      List<MilePart> roadpart, int[][] PBIVal, double[] SpeedVal, string[] MarkVal,
      int DataStartXlsxRow, int borderType)
        {
            string dirStr = prjinfo._Direction == 1 ? "右幅" : "左幅";
            ...
            object[,] vallist = new object[len, 14];
            for (int i = 0; i < len; i++)
            {
                vallist[i, 0] = prjinfo._RoadName;
                DateTime dateTime = DateTime.ParseExact(prjinfo._DataDate, "yyyyMMdd", CultureInfo.InvariantCulture);
                ...
                vallist[i, 6] = PBIVal[i][1];
                vallist[i, 7] = PBIVal[i][2];
                vallist[i, 8] = PBIVal[i][3];
                vallist[i, 9] = string.Format("=IF((100-G{0}*{1}-H{0}*{2}-I{0}*{3})>0,(100- G{0}*{1}-H{0}*{2}-I{0}*{3}),0)",
                    i + DataStartXlsxRow, _PBIScore[1], _PBIScore[2], _PBIScore[3]);
                vallist[i, 10] = string.Format("=IF(J{0}>={1},\"优\",IF(J{0}>={2},\"良\",IF(J{0}>={3},\"中\",IF(J{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow,
                    _PBIGrade[roadpart[i].roaddegree][0],
                    _PBIGrade[roadpart[i].roaddegree][1],
                    _PBIGrade[roadpart[i].roaddegree][2],
                ...
                vallist[i, 13] = MarkVal[i];
            }

            destrange = _Worksheet.get_Range(String.Format("A{0}:N{1}", DataStartXlsxRow, len + DataStartXlsxRow - 1));
            destrange.Value2 = vallist;
            GlobalExcel.SetBorderLine(destrange, borderType);

            if (_Setting.IsExcelSort)
            {
                MSExcel.Range destrange1 = _Worksheet.get_Range(string.Format("A4:N{0}", len + DataStartXlsxRow - 1));
                MSExcel.Range sortrange = _Worksheet.get_Range(string.Format("C4:C{0}", len + DataStartXlsxRow - 1));
                GlobalExcel.ReflectionColnum(_Worksheet, destrange1, sortrange);
            }
        }
        private static void WriteRut2Xls_orirut_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
        List<MilePart> roadpart, double[] LRutVal, double[] RRutVal, double[] SRutVal, double[] SpeedVal, string[] MarkVal,
        int DataStartXlsxRow, int borderType)
        {
            if (!prjinfo._IsRut)
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                vallist[i, 9] = string.Format("=IF(I{0}<{1},{2}-{3}*I{0},IF(I{0}<{4},{5}-{6}*(I{0}-{1}),0))",
                        i + DataStartXlsxRow, _RDIRD[0][1], _RDIRD[0][0], _RDIa[0], _RDIRD[1][1], _RDIRD[1][0], _RDIa[1]);
                vallist[i, 10] = string.Format("=IF(J{0}>={1},\"优\",IF(J{0}>={2},\"良\",IF(J{0}>={3},\"中\",IF(J{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow, _RDIGrade[roadpart[i].roaddegree][0], _RDIGrade[roadpart[i].roaddegree][1], _RDIGrade[roadpart[i].roaddegree][2], _RDIGrade[roadpart[i].roaddegree][3]);
                vallist[i, 11] = GlobalExcel._RoadTypeStr[roadpart[i].roadtype];
        ...
        private static void WriteRut2Xls_smtd_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
    List<MilePart> roadpart, double[] LMtdVal, double[] RMtdVal,  double[] SpeedVal, string[] MarkVal,
    int DataStartXlsxRow, int borderType)
        {
            if (!prjinfo._IsRut)
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                vallist[i, 6] = LMtdVal[i];

                if (prjinfo._IsDIRIMTD)
                {
                    vallist[i, 7] = RMtdVal[i];
                    vallist[i, 8] = String.Format("=ROUND((G{0}+H{0})/2,5)", i + DataStartXlsxRow);
                }
                else
                {
                    vallist[i, 8] = String.Format("=ROUND(G{0},5)", i + DataStartXlsxRow);
                }  
            ...
            if (_Setting.IsExcelSort)
            {
                MSExcel.Range destrange1 = _Worksheet.get_Range(string.Format("A4:L{0}", len + DataStartXlsxRow - 1));
                MSExcel.Range sortrange = _Worksheet.get_Range(string.Format("C4:C{0}", len + DataStartXlsxRow - 1));
                GlobalExcel.ReflectionColnum(_Worksheet, destrange1, sortrange);
            }
        }
        ...
        private static void WriteRut2XlsPwi_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
      List<MilePart> roadpart, double[] LMTDVal, double[] RMTDVal, double[] CMTDVal, double[] SpeedVal, string[] MarkVal,
      int DataStartXlsxRow, int borderType)
        {
            if (!prjinfo._IsRut)
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                {
                    vallist[i, 9] = string.Format("=IF(I{0}-MIN(G{0},H{0})>0, 100*(I{0}-MIN(G{0},H{0}))/I{0},0) ", i + DataStartXlsxRow);
                }

                vallist[i, 10] = string.Format("=100-{0}*POWER(J{1},{2})", _PWIa[0], i + DataStartXlsxRow, _PWIa[1]);
                vallist[i,11] = string.Format("=IF(K{0}>={1},\"优\",IF(K{0}>={2},\"良\",IF(K{0}>={3},\"中\",IF(K{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow,
                    _PWIGrade[roadpart[i].roaddegree][0],
                    _PWIGrade[roadpart[i].roaddegree][1],
                    _PWIGrade[roadpart[i].roaddegree][2],
                    _PWIGrade[roadpart[i].roaddegree][3]);
                vallist[i, 12] = GlobalExcel._RoadTypeStr[roadpart[i].roadtype];
                if (SpeedVal != null)
                {
                    vallist[i, 13] = SpeedVal[i];
                }
                vallist[i, 14] = MarkVal[i];
            }

            destrange = _Worksheet.get_Range(String.Format("A{0}:O{1}", DataStartXlsxRow, len + DataStartXlsxRow - 1));
            destrange.Value2 = vallist;
            GlobalExcel.SetBorderLine(destrange, borderType);

            if (_Setting.IsExcelSort)
            {
                MSExcel.Range destrange1 = _Worksheet.get_Range(string.Format("A4:O{0}", len + DataStartXlsxRow - 1));
                MSExcel.Range sortrange = _Worksheet.get_Range(string.Format("C4:C{0}", len + DataStartXlsxRow - 1));
                GlobalExcel.ReflectionColnum(_Worksheet, destrange1, sortrange);
            }
        }

        private static void WriteIRI2Xls_sichuan(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
       List<MilePart> roadpart, double[] LIRIVal, double[] RIRIVal, double[] SpeedVal, string[] MarkVal,
       int DataStartXlsxRow, int borderType)
        {
            if (!prjinfo._IsIRIMTD)
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                if (_Setting.RQIJudgeType == 0)
                {
                    vallist[i, 8] = String.Format("=ROUND(AVERAGE(G{0}:H{0}),5)", i + DataStartXlsxRow);
                }
                else if (_Setting.RQIJudgeType == 1)
                {
                    vallist[i, 8] = String.Format("=ROUND(MAX(G{0}, H{0}),5)", i + DataStartXlsxRow);
                }

                vallist[i, 9] = String.Format("=ROUND(100/(1+{0}*EXP({1}*I{2})),5)",
                    _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1], i + DataStartXlsxRow);
                vallist[i, 10] = string.Format("=IF(J{0}>={1},\"优\",IF(J{0}>={2},\"良\",IF(J{0}>={3},\"中\",IF(J{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow,
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][0],
        ...
        private static void WriteDisHZ2Xls_sichuan(MSExcel.Worksheet worksheet_snhz, MSExcel.Worksheet worksheet_lqhz,
          ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis,

          int DataStartXlsxRow, int borderType, ProcessOperator p)
        {
            MSExcel.Range destrange;
            int disnum = 0; 
            int rowcnt_lq_s = DataStartXlsxRow; 
            ...
            string errlog = prjdir.FullName + "\\errlog.txt";
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            object[,] disval = new object[len, 11 + 3];
            int typeidx = 0;
            bool res = false;
            string dirStr = prjinfo._Direction == 1 ? "右幅" : "左幅";
            int cumlLq = 0;
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                if (roadpart[i].roadtype == 0)//沥青
                {

                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = prjinfo._RoadName;
                    DateTime dateTime = DateTime.ParseExact(prjinfo._DataDate, "yyyyMMdd", CultureInfo.InvariantCulture);
                    string formattedString = dateTime.ToString("yyyy/MM/dd");
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = formattedString;
                    ...
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = dirStr;
                    ...
                    for (int di = 0, kk = 0; di < disnum; ++di,   ++kk)
                    {
                        if (di != 5 && di != 6)
                        {
                            disval[cumlLq, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][di].totalarea;
                    ...
                    disnum -= 2;
                    drval = ComputPCI_sichuan(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                    disval[cumlLq, disnum] = drval;
                    disval[cumlLq, disnum + 1] = string.Format("=100-{0}*POWER(R{1},{2})",
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        rowcnt_lq_s, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);

                    disval[cumlLq, disnum + 2] = string.Format("=IF(S{0}>={1},\"优\",IF(S{0}>={2},\"良\",IF(S{0}>={3},\"中\",IF(S{0}>={4},\"次\",\"差\"))))",
                        rowcnt_lq_s,
                        _PCIGrade[roadpart[i].roaddegree][0],
                        _PCIGrade[roadpart[i].roaddegree][1],
                        _PCIGrade[roadpart[i].roaddegree][2],
                ...
                p._backgroundWorker.ReportProgress(95 * i / len);
            }

            destrange = worksheet_lqhz.get_Range(string.Format("G{2}:{1}{0}", rowcnt_lq_s - 1, GlobalExcel.GetCol((char)('G' + disnum + 2)), 5));
            destrange.Value2 = disval;

            destrange = worksheet_lqhz.get_Range(String.Format("A1:T{0}", rowcnt_lq_s - 1));
            GlobalExcel.SetBorderLine(destrange, borderType);
            // destrange = worksheet_snhz.get_Range(String.Format("A1:R{0}", rowcnt_sn_s));
            // GlobalExcel.SetBorderLine(destrange, borderType);
            ...
            if (_Setting.IsExcelSort)
            {
                 MSExcel.Range  destrange1 = worksheet_lqhz.get_Range(string.Format("A5:T{0}", rowcnt_lq_s - 1));
                MSExcel.Range sortrange = worksheet_lqhz.get_Range(string.Format("C5:C{0}", rowcnt_lq_s - 1));
                GlobalExcel.ReflectionColnum(worksheet_lqhz, destrange1, sortrange); 
            }
        }

        public static void OutputRoad_Szechwan(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval, List<int> userSelectd, ProcessOperator p)
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\四川公路院\公路院导出数据模板_small.xlsx",
                System.Windows.Forms.Application.StartupPath);
            string Destxls = string.Format(@"{0}\{1}_{2}m.xlsx", path, prjdir.Name, disval);
            MSExcel.Workbook _Workbook = excelApp.Workbooks.Open(srcxls, Type.Missing,
                true, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
            ...
            foreach (var index in userSelectd)
            {
                switch (index)
                {
                    case 0: //pci
                        MSExcel.Worksheet _Worksheet_lqhz = _Workbook.Sheets["PCI"] as MSExcel.Worksheet;
                        MSExcel.Worksheet _Worksheet_snhz = null;
                        WriteDisHZ2Xls_sichuan(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, 5, 53, p);
                        break;
                    case 1: //病害
                        MSExcel.Worksheet _Worksheet_D = _Workbook.Sheets["病害列表"] as MSExcel.Worksheet;
                        WriteDisLB2Xls_roadpart_sichuan(_Worksheet_D, prjinfo, prjdir, _SmallRoadDisList, _RoadPart);
                       // p._backgroundWorker.ReportProgress(100);
                        break;
                    case 2: //rqi
                        MSExcel.Worksheet _Worksheet_Rqi = _Workbook.Sheets["RQI"] as MSExcel.Worksheet;
                        WriteIRI2Xls_sichuan(_Worksheet_Rqi, prjinfo, _RoadPart, _LIRIMeanVal, _RIRIMeanVal, _SpeedVal, _MarkVal, 4, 53);

                        if (_Setting.Out_roadinfo == 0)
                        {
                            ((MSExcel.Range)_Worksheet_Rqi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                            ((MSExcel.Range)_Worksheet_Rqi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                        }
                        break;
                    case 3: //rdi
                        MSExcel.Worksheet _Worksheet_Rdi = _Workbook.Sheets["RDI"] as MSExcel.Worksheet;
                        WriteRut2Xls_orirut_sichuan(_Worksheet_Rdi, prjinfo, _RoadPart, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal, _SpeedVal, _MarkVal, 4, 53);
                        p._backgroundWorker.ReportProgress(96);
                        if (_Setting.Out_roadinfo == 0)
                        {
                            ((MSExcel.Range)_Worksheet_Rdi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                            ((MSExcel.Range)_Worksheet_Rdi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                        }
                        break;
                    case 4: //pbi
                        MSExcel.Worksheet _Worksheet_Pbi = _Workbook.Sheets["PBI"] as MSExcel.Worksheet;
                        WritePBI2Xls_sichuan(_Worksheet_Pbi, prjinfo, prjdir, _RoadPart, _PBIVal, _SpeedVal, _MarkVal, 4, 53);
                        if (_Setting.Out_roadinfo == 0)
                        {
                            ((MSExcel.Range)_Worksheet_Pbi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                            ((MSExcel.Range)_Worksheet_Pbi.Cells[System.Reflection.Missing.Value, 13]).EntireColumn.Delete();
                        }

                        break;
                    case 5:  //Δh
                        MSExcel.Worksheet _Worksheet_other = _Workbook.Sheets["Δh"] as MSExcel.Worksheet;
                        WritePB2Xls_sichuan(_Worksheet_other, prjinfo, prjdir, _RoadPart10, _LDeltaHVal, _RDeltaHVal, _SpeedVal10, _MarkVal10, 4);
                        p._backgroundWorker.ReportProgress(98);
                        break;
                    case 6:  //pwi
                        MSExcel.Worksheet _Worksheet_pwi = _Workbook.Sheets["PWI"] as MSExcel.Worksheet;
                        WriteRut2XlsPwi_sichuan(_Worksheet_pwi, prjinfo, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, _SpeedVal, _MarkVal, 4, 53);
                        if (_Setting.Out_roadinfo == 0)
                        {
                            ((MSExcel.Range)_Worksheet_pwi.Cells[System.Reflection.Missing.Value, 14]).EntireColumn.Delete();
                            ((MSExcel.Range)_Worksheet_pwi.Cells[System.Reflection.Missing.Value, 14]).EntireColumn.Delete();
                        }
                        p._backgroundWorker.ReportProgress(98);
                        break;
                    case 7:  //smtd
                        MSExcel.Worksheet _Worksheet_SMTD = _Workbook.Sheets["SMTD"] as MSExcel.Worksheet;
                        WriteRut2Xls_smtd_sichuan(_Worksheet_SMTD, prjinfo, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _SpeedVal, _MarkVal, 4, 53);
                        if (_Setting.Out_roadinfo == 0)
                        {
                            ((MSExcel.Range)_Worksheet_SMTD.Cells[System.Reflection.Missing.Value, 11]).EntireColumn.Delete();
                            ((MSExcel.Range)_Worksheet_SMTD.Cells[System.Reflection.Missing.Value, 11]).EntireColumn.Delete();
                        }
                        p._backgroundWorker.ReportProgress(98);
            ...
            List<string> sheets = new List<string>() { "PCI", "病害列表", "RQI", "RDI", "PBI", "Δh", "PWI", "SMTD" };
            for (int i = 0; i < 8; i++)
            {
                if (!userSelectd.Contains(i))
                {
                    string sheetName = sheets[i];  // 获取要删除的Sheet对象
                    MSExcel.Worksheet sheetToDelete = null;
                    // 获取要删除的Sheet对象 
        ...
        private static void P_BackgroundWorkerCompleted(object sender, EventArgs e)
        {
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\多车道统计-Small.xlsx",
                System.Windows.Forms.Application.StartupPath);
        ...
        private static void WriteBHMJ2Xlsx(MSExcel.Application excelApp, MSExcel.Worksheet destsheet, List<string> BHTJxlslist)
        {
            ...
            foreach (string tlane in BHTJxlslist)
            {
                ...
                if (tsheet != null)
                {
                    ...
                    trange = tsheet.get_Range("A1:S" + userow.ToString());
                    tobj = (object[,])trange.Value2;
                ...
                if (islq)
                {
                    ...
                    trange = destsheet.get_Range(string.Format("M{0}:Y{0}", rowidx));
                    trange.Value2 = lqobj;
                    trange = destsheet.get_Range(string.Format("Z{0}:AK{0}", rowidx));
                    trange.Value2 = snobj;
                    ++rowidx;
                }
                ...
                if (tsheet != null)
                {
                    issn = true;
                    userow = GlobalExcel.judegeusedrow(tsheet, 4, 5);
                    trange = tsheet.get_Range("A1:R" + userow.ToString());
                    tobj = (object[,])trange.Value2;
                    for (int i = 0; i < sndisnum; ++i)
                    {
        ...
        public static void OutputPBI_XMJH(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            WritePB2Xls(_Worksheet, prjinfo, prjdir, _RoadPart10, _LDeltaHVal, _RDeltaHVal, _SpeedVal10, _MarkVal10, 11);
            _Workbook.Save();
            _Workbook.Close(Type.Missing, Type.Missing, Type.Missing);
        ...
        public static void OutputPQI_XMJH(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            WritePQI2Xls_XMJH(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
                _LIRIMeanVal, _RIRIMeanVal, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal,
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\厦门捷航\报告模板\路面破损（图像法-图像检测法）PCI试验检测报告.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            WriteDisHZ2Xls(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, ref Haslqflag, ref Hassnflag, 20, 63);
            _Workbook.Save();
            _Workbook.Close(Type.Missing, Type.Missing, Type.Missing);

            srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\厦门捷航\记录模板\路面破损（图像法-图像检测法）试验检测记录.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            WriteDisLB2Xls_roadpart_XMJH(_Worksheet_DisLB, prjinfo, prjdir, _SmallRoadDisList, _RoadPart);
            _Workbook.Save();
            _Workbook.Close(Type.Missing, Type.Missing, Type.Missing);
        ...
        private static void WriteDisLB2Xls_roadpart_XMJH(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir, SmalRectDisease[] arrdis, List<MilePart> roadpart)
        {
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            if (len < 1 || dlen < 1)
                return;
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                    ...
                    || (prjinfo._Direction < 0 && arrdis[j].m_mile <= smile && arrdis[j].m_mile > emile)))
                {
                    ...
                    if (res)
                    {
                        ...
                        vallist[rowcnt, 4] = arrdis[j].Area;
                        vallist[rowcnt, 5] = arrdis[j].imgname;
                        vallist[rowcnt, 6] = arrdis[j].imgpath;
            ...
            MSExcel.Range destrange = _Worksheet.get_Range(String.Format("A12:I{0}", dlen + 11));
            destrange.Value2 = vallist;
            GlobalExcel.SetBorderLine(destrange, 63);

            if (_Setting.IsExcelSort)
            {
                GlobalExcel.Reflection(_Worksheet, 12, 1, 9, true);
            }
        }
        #endregion
        ...
        public static void OutputPQI_HNJZ_ZHPD(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            Write_HNJZ_ZHPD(worksheet_pqi, worksheet_pci, worksheet_rqi, worksheet_rdi, worksheet_pbi, worksheet_pwi,
                prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
        ...
        public static void OutputDis_HPcsv_0(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            string destxls = string.Format(@"{0}\{2}-{3} {1} {4}{5}-黑色-{6}m.csv", path, prjinfo._RoadCode, prjdir.Name, strDirection, prjinfo._DataDate, prjinfo._DataTime,disval);
            ...
            bool Haslqflag = false;
            WriteDisHZ2Xls_modle10(_Worksheet_snhz, prjinfo, prjdir, _RoadPart1M, _SmallRoadDisList, ref Haslqflag, 53, prjinfo._RoadImgDis, "Road");
        ...
        public static void OutputDis_HPcsv_1(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            MSExcel.Workbook _WorkbookSN = null;
            // MSExcel.Workbook _WorkbookLQ = null;
            MSExcel.Worksheet _Worksheet_snhz = null;
            ...
            string destxls = string.Format(@"{0}\{2}-{3} {1} {4}{5}-iri-{6}.csv", path, prjinfo._RoadCode, prjdir.Name, strDirection, prjinfo._DataDate, prjinfo._DataTime,disval);
            string destxlsTxt = string.Format(@"{0}\{2}-{3} {1} {4}{5}-iri-{6}.txt", path, prjinfo._RoadCode, prjdir.Name, strDirection, prjinfo._DataDate, prjinfo._DataTime,disval);
            ...
            _WorkbookSN.SaveAs(destxls, MSExcel.XlFileFormat.xlCSV, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
              MSExcel.XlSaveAsAccessMode.xlNoChange, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            bool Haslqflag = false;
            //  WriteDisHZ2Xls_modle1_1(_Worksheet_snhz, prjinfo, prjdir, _RoadPart1M, _SmallRoadDisList, ref Haslqflag, 53, prjinfo._RoadImgDis, "Road", destxlsTxt);
            WriteDisHZ2Xls_modle1_1(_Worksheet_snhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, ref Haslqflag, 53, prjinfo._RoadImgDis, "Road", destxlsTxt);
        ...
        public static void OutputDis_HPcsv_Rut(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            string destxls = string.Format(@"{0}\{2}-{3} {1} {4}{5}-rut-{6}.csv", path, prjinfo._RoadCode, prjdir.Name, strDirection, prjinfo._DataDate, prjinfo._DataTime,disval);
            string destxlsTxt = string.Format(@"{0}\{2}-{3} {1} {4}{5}-rut-{6}.txt", path, prjinfo._RoadCode, prjdir.Name, strDirection, prjinfo._DataDate, prjinfo._DataTime,disval);
        ...
        public static void outPutDrCsv_WH(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir)
        {
            ...
            SmallWritePCI2XlsCsv_WH(_Worksheet_iri, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);
        ...
        public static void OutputDis_HPcsv_2(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            bool Haslqflag = false;
            WriteDisHZ2Xls_modle1_2(_Worksheet_snhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, ref Haslqflag, 53, prjinfo._RoadImgDis, "Road");
        ...
        private static void WriteDisHZ2Xls_modle10(MSExcel.Worksheet worksheet_snhz,
         ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart2M, SmalRectDisease[] arrdis,
         ref bool Haslqflag, int borderType, int ImgDis, string ImgType)
        {
            ...
            var temp = from a in RoadPart2M
                       where leftimgsinfos.Contains(a.mile.ToString())
                       select a;

            RoadPart2M = temp.ToList();
            ...
            string errlog = prjdir.FullName + "\\errlog.txt";
            int len = RoadPart2M.Count - 1, dlen = arrdis.Length;
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                double smile = RoadPart2M[i].mile;
                double emile = RoadPart2M[i + 1].mile;
                //int emile = RoadPart1M[i + 1].mile;
                //int milelength = Math.Abs(smile - emile);
                disvalsn[i, colcnt++] = prjinfo._RoadCode;//路线代码
                disvalsn[i, colcnt++] = prjinfo._DataDate;
                ...
                if (dicGps.Keys.Contains(RoadPart2M[i].mile))
                {
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile]._latitude;
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile]._longitude;
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile]._elevation;
                }
                else if (dicGps.Keys.Contains(RoadPart2M[i].mile + 1))
                {
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile + 1]._latitude;
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile + 1]._longitude;
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile + 1]._elevation;
                }
                else
                {
                    disvalsn[i, colcnt++] = " ";
                ...
                while (j < dlen && ((prjinfo._Direction > 0 && arrdis[j].m_mile >= smile && arrdis[j].m_mile < emile)
                    || (prjinfo._Direction < 0 && arrdis[j].m_mile <= smile && arrdis[j].m_mile > emile)))
                {
                    res = RoadDiseaseTypes.DiseaseTypeDict[RoadPart2M[i].roadtype].TryGetValue(string.Format("{0}.{1}",
                        arrdis[j].RoadType, arrdis[j].RoadDisType), out typeidx);
                    if (res)
                    {
                        // disvalsn[i, colcnt++] = RoadDiseaseTypes.roaddis[RoadPart2M[i].roadtype][typeidx].disname ?? "";
                        // disvalsn[i, colcnt++] = RoadDiseaseTypes.roaddis[RoadPart2M[i].roadtype][typeidx].degree ?? "";
                        //disvalsn[i, colcnt++] = RoadDiseaseTypes.roaddis[RoadPart2M[i].roadtype][typeidx].totallength;
                        //disvalsn[i, colcnt++] = (double)arrdis[j].Area;

                        string name = RoadDiseaseTypes.roaddis[RoadPart2M[i].roadtype][typeidx].disname;
                        int roadType = RoadPart2M[i].roadtype;
                        switch (roadType)
                        {
                            case 0:
                                if (name.Contains("龟裂"))
                    ...
                    {
                        string errval = string.Format("拉框病害信息：{0} 路面材质：{1}\r\n", arrdis[j].GetDisInfoStr(), GlobalExcel._RoadTypeStr[RoadPart2M[i].roadtype]);
                        File.AppendAllText(errlog, errval, Encoding.UTF8);
                    }
                    ++j;
                    //  RoadDiseaseTypes.roaddis[roadpart[i].roadtype][typeidx].totalarea += arrdis[j].Area;
            ...
            if (Haslqflag)
            {
                ...
                else if (_Setting.IsExcelSort && prjinfo._Direction < 0)
                {
                    ...
                    destrange = worksheet_snhz.get_Range(String.Format("A2:AE{0}", rowcnt_sn + 1));
                    MSExcel.Range sortrange = worksheet_snhz.get_Range(String.Format("I2:I{0}", len + 1));//按桩号排序
                    GlobalExcel.ReflectionColnumDescending(worksheet_snhz, destrange, sortrange);
          ...
          ref bool Haslqflag, int borderType, int ImgDis, string ImgType, string textPath)
        {
            ...
            if (prjinfo._IsIRIMTD)
            {
                GlobalExcel.GetSpeedMeanVal(prjinfo, prjdir, RoadPart2M, ref _SpeedVal);
                GlobalExcel.GetIRIMeanVal(prjinfo, prjdir, RoadPart2M, ref IRI_LIRIVal, ref IRI_RIRIVal, _Setting.IsWarning);
            ...
            for (int i = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                disvalsn[i, colcnt++] = _SpeedVal[i];//速度 (千米/小时)
                if (dicGps.Keys.Contains(RoadPart2M[i].mile))
                {
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile]._latitude;
                    disvalsn[i, colcnt++] = dicGps[RoadPart2M[i].mile]._longitude;
                ...
                string[] format = { "yyyyMMddHHmmss" };
                DateTime resDateTime;
                if (DateTime.TryParseExact(prjinfo._DataDate + prjinfo._DataTime, format, CultureInfo.CurrentCulture, System.Globalization.DateTimeStyles.None, out resDateTime))
                {
                    date = resDateTime.ToString("yyyy/MM/dd");
                    disvalsn[i, colcnt++] = date;//测量日
                    date = resDateTime.ToString("HH:mm:ss");
        ...
        ref bool Haslqflag, int borderType, int ImgDis, string ImgType)
        {
            ...
            if (prjinfo._IsIRIMTD)
            {
                ...
                //GlobalExcel.GetAllMilePart(prjdir.FullName, prjinfo, 10, prjinfo._Direction, _RoadGradeStr, ref _RoadPart10, RoadDiseaseTypes.roadtypedict, _RoadGradeDict);
                GlobalExcel.GetDeltaHVal(prjinfo, prjdir, RoadPart2M, 0, ref _LDeltaHVal);
                if (prjinfo._IsDIRIMTD)
                {
                    GlobalExcel.GetDeltaHVal(prjinfo, prjdir, RoadPart2M, 1, ref _RDeltaHVal);
                }
                GlobalExcel.GetPBVal(prjinfo, prjdir, _RoadPart, RoadPart2M, ref _PBIVal, _PBIThresh, _LDeltaHVal, _RDeltaHVal, 0, ref _DeltaHVal);
                GlobalExcel.GetMarkInfo(prjinfo, prjdir, RoadPart2M, ref _MarkVal10);
                GlobalExcel.GetSpeedMeanVal(prjinfo, prjdir, RoadPart2M, ref _SpeedVal10);
                GlobalExcel.calculateAcceleratedSpeed(RoadPart2M, _SpeedVal, dicGps, ref cA);
            }
            int len = RoadPart2M.Count - 1, dlen = arrdis.Length;
            // object[,] disvallq = new object[len, 30];
            ...
            for (int i = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                double RDeltaHVal = _RDeltaHVal[i];
                ...
                disvalsn[i, colcnt++] = RDeltaHVal.ToString("f3");
                disvalsn[i, colcnt++] = LDeltaHVal.ToString("f3");
            }
            if (Haslqflag)
            {
        ...
        public static void OutputDis_CICS(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            string strDirection = prjinfo._Direction > 0 ? "A" : "B";
            string[] _RoadTypeStr = { "沥青", "水泥", "砂石" };
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\CICS\{1}\水泥混凝土路面破损{1}汇总.xlsx", System.Windows.Forms.Application.StartupPath, subdname);
            string destxls = string.Format(@"{0}\{1}{2}{3}({4}{5}{3}(已识别)-路面破损-水泥路面).csv", path, prjinfo._RoadCode, prjinfo._RoadName, strDirection, prjinfo._DataDate, prjinfo._DataTime);
            // string destxls = string.Format(@"{0}\{1}_水泥混凝土路面破损{2}汇总.csv", path, prjdir.Name, subdname);
            ...
            destxls = string.Format(@"{0}\{1}{2}{3}({4}{5}{3}(已识别)-路面破损-沥青路面).csv", path, prjinfo._RoadCode, prjinfo._RoadName, strDirection, prjinfo._DataDate, prjinfo._DataTime);
            // destxls = string.Format(@"{0}\{1}_沥青路面破损{2}汇总.csv", path, prjdir.Name, subdname);
            ...
            _WorkbookLQ.SaveAs(destxls, MSExcel.XlFileFormat.xlCSV, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
               MSExcel.XlSaveAsAccessMode.xlNoChange, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            bool Haslqflag = false;
            bool Hassnflag = false;
            WriteDisHZ2Xls_CICS(_Worksheet_snhz, _Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, ref Haslqflag, ref Hassnflag, 53);

            if (!Haslqflag)
            {
                _WorkbookLQ.Close(Type.Missing, Type.Missing, Type.Missing);
                File.Delete(string.Format(@"{0}\{1}{2}{3}({4}{5}{3}(已识别)-路面破损-沥青路面).csv", path, prjinfo._RoadCode, prjinfo._RoadName, strDirection, prjinfo._DataDate, prjinfo._DataTime));
            }
            else
            {
                _WorkbookLQ.Save();
            ...
            {
                _WorkbookSN.Close(Type.Missing, Type.Missing, Type.Missing);
                File.Delete(string.Format(@"{0}\{1}{2}{3}({4}{5}{3}(已识别)-路面破损-水泥路面).csv", path, prjinfo._RoadCode, prjinfo._RoadName, strDirection, prjinfo._DataDate, prjinfo._DataTime));
            }
            else
            {
                _WorkbookSN.Save();
            ...
            ref bool Haslqflag, ref bool Hassnflag, int borderType)
        {
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                {
                    Hassnflag = true;
                    disvalsn[rowcnt_sn, colcnt++] = (double)smile / 1000.0;
                    disvalsn[rowcnt_sn, colcnt++] = (double)emile / 1000.0;
                    ...
                    disvalsn[rowcnt_sn, colcnt++] = 0;
                    disvalsn[rowcnt_sn, colcnt++] = 0;
                    drval = ComputPCI(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                    disvalsn[rowcnt_sn, colcnt++] = drval;
                    double tmp = 100 - _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0] * Math.Pow(drval, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);
                    if (tmp < 0) tmp = 0;
                    disvalsn[rowcnt_sn, colcnt++] = tmp;
                    disvalsn[rowcnt_sn, colcnt++] = 2.7;
                    ++rowcnt_sn;
                }
                else if (roadpart[i].roadtype == 0)//沥青
                {
                    Haslqflag = true;
                    disvallq[rowcnt_lq, colcnt++] = (double)smile / 1000.0;
                    disvallq[rowcnt_lq, colcnt++] = (double)emile / 1000.0;
                    ...
                    for (int kk = 0; kk < lqdis_name.Length; ++kk)
                    {
                        ...
                        if (type.fulltype == "沥青.车辙.轻")
                        {
                            chezhe = type.totalarea;
                            continue;
                        }
                        else if (type.fulltype == "沥青.车辙.重")
                        {
                            chezhe += type.totalarea;
                            disvallq[rowcnt_lq, colcnt++] = chezhe;
                            continue;
                        }
                        else
                        {
            ...
            if (Haslqflag)
            {
                ...
                {
                    destrange = worksheet_lqhz.get_Range(String.Format("A2:O{0}", rowcnt_lq + 1));
                    MSExcel.Range sortrange = worksheet_lqhz.get_Range(String.Format("B2:B{0}", len + 1));//按桩号排序
                    GlobalExcel.ReflectionColnumDescending(worksheet_lqhz, destrange, sortrange);
            ...
            if (Hassnflag)
            {
                ...
                else if (_Setting.IsExcelSort && prjinfo._Direction < 0)
                {
                    ...
                    destrange = worksheet_snhz.get_Range(String.Format("A2:O{0}", rowcnt_sn + 1));
                    MSExcel.Range sortrange = worksheet_snhz.get_Range(String.Format("B2:B{0}", len + 1));//按桩号排序
                    GlobalExcel.ReflectionColnumDescending(worksheet_snhz, destrange, sortrange);
  ...
  List<MilePart> roadpart, double[] LIRIVal, double[] RIRIVal, int DataStartXlsxRow, int borderType)
        ...
        public static void outPutAutoTest_9(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir)
        {
            ...
            string Destxls = string.Format(@"{0}\{1}{2}-LOC-{3}-标准格式-{4}{5}.txt", path,prjinfo._RoadCodePart
                ,prjinfo._Direction==1?"A":"B",prjinfo._StartMile/1000.0,prjinfo._DataDate,prjinfo._DataTime);
            ExcelGPS[] dicGps = null; 
            GlobalExcel.GetGPSInfo(prjinfo, prjdir, _RoadPart, ref dicGps);
            int len = _RoadPart.Count; 
            List<string> datas = new List<string>() { "脉冲,桩号(km),x(m),y(m),z(m),有效性" }; 
            //2m-2000
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                int index = 1000*Math.Abs( smile - _RoadPart[0].mile);
                string line = string.Join(",", index, (smile / 1000.0).ToString("f3"), dicGps[i]._longitude,
                    dicGps[i]._latitude, dicGps[i]._elevation, "A");
                datas.Add(line);
            }
     ...
     double[] LMTDVal, double[] RMTDVal, double[] MMTDVal, int[][] PBVal, double[] deltahVal, int disval, ExcelGPS[] gpsInfo)
        {
            ...
            object[,] mxlist = new object[len, 26];
            ...
            for (int i = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                mxlist[i, 11] = milelength1;
                mxlist[i, 12] = roadpart[i].width;
                mxlist[i, 13] = prjinfo._City;
                mxlist[i, 14] = prjinfo._District;
                mxlist[i, 15] = roadpart[i].unit;
                if (roadpart[i].isPub)
                {
                    mxlist[i, 16] = "共有路段";
                }
                ...
                {
                    mxlist[i, 16] = roadpart[i].degreestr;
                }

                mxlist[i, 17] = roadpart[i].roadtype == 0 ? "沥青" : roadpart[i].roadtype == 1 ? "水泥" : "砂石";
                ...
                tpcival = 100 - _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0] * Math.Pow(drval, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);
                drvals[i] = drval;   //dr
                mxlist[i, 6] = string.Format("=100-{0}*POWER(V{1},{2})", _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], i + 2, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]); //pci
                mxlist[i, 19] = string.Format("=IF(G{0}>={1},\"优\",IF(G{0}>={2},\"良\",IF(G{0}>={3},\"中\",IF(G{0}>={4},\"次\",\"差\"))))",
                ...
                if (prjinfo._IsRut)
                {
                    try
                    {
                        ...
                        // if(roadpart[i].roadtype==)
                        mxlist[i, 8] = Math.Round(rdival, 5);



                        mxlist[i, 5] = string.Format("=ROUND(({1}*G{0}+{2}*H{0}+{3}*I{0})/({1}+{2}+{3}),5)",i + 2,
                        _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][1],
                        _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][2]
                        );
                    ...
                    catch (Exception)
                    {
                        ...
                        mxlist[i, 8] = 0.0;
                        mxlist[i, 5] = string.Format("=ROUND(({1}*G{0}+{2}*H{0})/({1}+{2}),5)", i + 2,
                       _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                       _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][1]
                       );
                    }
                ...
                mxlist[i, 20] = string.Format("=IF(H{0}>={1},\"优\",IF(H{0}>={2},\"良\",IF(H{0}>={3},\"中\",IF(H{0}>={4},\"次\",\"差\"))))",
                    i + 2,
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][1],
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][2],
                ...
                //PQI 评价
                mxlist[i, 18] = string.Format("=IF(F{0}>={1},\"优\",IF(F{0}>={2},\"良\",IF(F{0}>={3},\"中\",IF(F{0}>={4},\"次\",\"差\"))))",
                i + 2,
                _PQIGrade[roadpart[i].roaddegree][0],
                _PQIGrade[roadpart[i].roaddegree][1],
                _PQIGrade[roadpart[i].roaddegree][2],
            ...
            if (_Setting.IsExcelSort)
            {
                MSExcel.Range sortrange = worksheet.get_Range(String.Format("J2:J{0}", len + 1));
                GlobalExcel.ReflectionColnum(worksheet, destrange, sortrange);
            }
            //处理过小分段问题
            ...
            #region 处理过小分段问题 
            {
                ...
                if (_Setting.hefei2MinSplit > 0)
                {
                    ...
                    for (int i = 1; i <= len; i++)
                    {
                        object[] preValue = new object[27];
                        object[] nowValue = new object[27];
                        if (i == len)
                        {
                            for (int t = 1; t < 27; t++)
                            {
                            ...
                            else
                            {
                                ...
                                double preWidth = double.Parse(preValue[13].ToString());
                                
                                string preUnit = preValue[16] == null? "": preValue[16].ToString();
                                string preGrad = preValue[17].ToString();
                                string preType = preValue[18].ToString();
                                double preIRI = double.Parse(preValue[23].ToString());
                                double preDr = double.Parse(preValue[22].ToString());

                                double nowLen = double.Parse(nowValue[12].ToString());

                                double nowWidth = double.Parse(nowValue[13].ToString());
                                if (nowWidth == 0)
                                {
                                    nowWidth = preWidth;
                                    nowValue[13] = preWidth;
                                }
                                string nowUnit = "";
                                if (nowValue[16] == null)
                                {
                                ...
                                {
                                    nowUnit = nowValue[16].ToString();
                                }

                                string nowGrad = nowValue[17].ToString();
                                string nowType = nowValue[18].ToString();
                                double nowIRI = double.Parse(nowValue[23].ToString());
                                double nowDr = double.Parse(nowValue[22].ToString());

                                double preLen = double.Parse(preValue[12].ToString());
                                //pqi  
                                ...
                                double preRut = double.Parse(preValue[9].ToString());
                                ...
                                double nowRut = double.Parse(nowValue[9].ToString());
                                ...
                                if ((nowLen * 1000) <= splitValue)
                                {
                                    if (preWidth == nowWidth && preUnit == nowUnit && preGrad == nowGrad && preType == nowType)
                                    {
                                        //第一分段和第二分段合并 
                                        //如果下一个分段小于指定值 
                                        resultValues[resultValues.Count - 1][11] = nowValue[11];
                                        resultValues[resultValues.Count - 1][11] = double.Parse(resultValues[resultValues.Count - 1][11].ToString()) - double.Parse(resultValues[resultValues.Count - 1][10].ToString());
                                        //定位
                                        resultValues[resultValues.Count - 1][25] = nowValue[25];

                                        double newIRI = ((preIRI * preLen) + (nowIRI * nowLen)) / (preLen + nowLen);
                                        resultValues[resultValues.Count - 1][23] = newIRI;
                                        ...
                                        double rut = ((preRut * preLen) + (nowRut * nowLen)) / (preLen + nowLen);
                                        resultValues[resultValues.Count - 1][9] = rut;

                                        resultValues[resultValues.Count - 1][22] = newDr;
                                ...
                                else if ((preLen * 1000) <= splitValue)
                                {
                                    if (preWidth == nowWidth && preUnit == nowUnit && preGrad == nowGrad && preType == nowType)
                                    {
                                        ...
                                        //如果下一个分段小于指定值 
                                        resultValues[resultValues.Count - 1][11] = nowValue[11];
                                        resultValues[resultValues.Count - 1][12] = double.Parse(resultValues[resultValues.Count - 1][11].ToString()) - double.Parse(resultValues[resultValues.Count - 1][10].ToString());
                                        //定位
                                        resultValues[resultValues.Count - 1][25] = nowValue[25];
                        ...
                        else
                        {
                            ...
                            double nowLen = double.Parse(nowValue[12].ToString());
                            double nowWidth = 0;
                            if (double.Parse(nowValue[13].ToString()) == 0)
                            {
                                nowWidth = preWidth;
                                nowValue[13] = preWidth;
                            }
                            ...
                            {
                                nowWidth = double.Parse(nowValue[13].ToString());
                            }
                            string nowUnit = "";
                            if (nowValue[16] == null)
                            {
                            ...
                            double nowRut = double.Parse(nowValue[8].ToString());
                            ...
                            if ((preLen * 1000) <= splitValue)
                            {
                                if (resultValues.Count == 0)
                                {
                                    ...
                                    if (preWidth == nowWidth && preUnit == nowUnit && preGrad == nowGrad && preType == nowType)
                                    {
                                        //第一分段和第二分段合并 
                                        //如果下一个分段小于指定值 
                                        preValue[11] = nowValue[11];
                                        preValue[12] = double.Parse(preValue[11].ToString()) - double.Parse(preValue[10].ToString());
                                        preValue[25] = nowValue[25];
                                ...
                                else
                                {
                                    ...
                                    //前一个变成了列表最后一项 
                                    preValue = resultValues.Last();
                                    preWidth = double.Parse(preValue[13].ToString());
                                    preUnit = preValue[16].ToString();
                                    preGrad = preValue[17].ToString();
                                    preType = preValue[18].ToString();
                                    preIRI = double.Parse(preValue[23].ToString());
                                    preDr = double.Parse(preValue[22].ToString());
                                    nowLen = double.Parse(nowValue[12].ToString());
                                    nowWidth = double.Parse(nowValue[13].ToString());
                                    nowUnit = nowValue[16].ToString();
                                    nowGrad = nowValue[17].ToString();
                                    nowType = nowValue[18].ToString();
                                    nowIRI = double.Parse(nowValue[23].ToString());
                                    nowDr = double.Parse(nowValue[22].ToString());
                                    ...
                                    preRut = double.Parse(preValue[9].ToString());
                                    //pqi  
                                    ...
                                    nowRut = double.Parse(nowValue[9].ToString());
                                    ...
                                    if (preWidth == nowWidth && preUnit == nowUnit && preGrad == nowGrad && preType == nowType)
                                    {
                                        ...
                                        double rqi = ((preRqi * preLen) + (nowRqi * nowLen)) / (preLen + nowLen);
                                        resultValues[resultValues.Count - 1][8] = rqi;
                                        double rut = ((preRut * preRut) + (nowRut * nowLen)) / (preRut + nowLen);
                                        resultValues[resultValues.Count - 1][9] = rut;
                            ...
                            else
                            {
                                if (resultValues.Count > 0)
                                {
                                    ...
                                    preLen = double.Parse(preValue[12].ToString());
                                    if ((preLen * 1000) <= splitValue)
                                    {
                                        if (preWidth == nowWidth && preUnit == nowUnit && preGrad == nowGrad && preType == nowType)
                                        {
                ...
                object[,] ResultMxlist = new object[resultValues.Count, 26];
                ...
                destrange.Clear();
                destrange = worksheet.get_Range(String.Format("A2:Z{0}", resultValues.Count + 1));
        ...
        public static void Convent_Acc(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, string fileName)
        {
            ...
            if (prjinfo._IsIRIMTD)
            {
                GlobalExcel.calculateAcceleratedSpeed(_RoadPart, _SpeedVal, dicGps, ref cA);
        ...
        public static void Convent_Mpd(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, string fileName)
        {
            ...
            MSExcel.Worksheet _Worksheet_ = _Workbook.Sheets["sheet1"] as MSExcel.Worksheet;
            writeAutoTestXls_Mpd(_Worksheet_, prjinfo, prjdir, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, 4);
   ...
   bool IsDis, bool IsMeanIRI, bool IsMeanMTD, bool IsMeanRut, bool IsPBI, bool IsSpeed, bool IsMeanMPD = false)
        ...
        public static void Convent_LP(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, string fileName)
        {
            ...
            MSExcel.Worksheet _Worksheet_ = _Workbook.Sheets["sheet1"] as MSExcel.Worksheet;
            // writeAutoTestXls_gc_gj(_Worksheet_, _RoadPart, prjdir, prjinfo, _SpeedVal);
        ...
        public static void Convent_Damage_JiangXi(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval, string fileName)
        {
            ...
            foreach (string roadTypeInfo in roadTypeSplit)
            {
                ...
                WriteDis_Damage2Xls_gj_JiangXi(_Worksheet_snhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, 53, i, col, startMileD, endMileStrD);
    ...
    ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int borderType, int roadType, int cluCount, double sMile, double eMile)
        ...
        private static void OutDisPart(string path, string fileName, int rowCount, object[,] datas, string type)
        {
     ...
     ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int borderType, int roadType, int cluCount, ref object[,] allDatas, ref bool has, out int temp)
        {
            MSExcel.Range destrange;
...
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M, double[] SpeedVal, double[] cA, int lastCol)
        ...
        public static void OutputSNDamage(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            ...
            bool has = false;
            WriteDisLQDamage2Xls_gj(_Worksheet_lqhz, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, 53, 1, 7, ref has);
            if (has)
            {
                _Workbook.Save();
                _Workbook.Close(Type.Missing, Type.Missing, Type.Missing);
        ...
        /// <param name="roadType">0沥青 1水泥</param>
        /// rowCount 开始行数
        private static void WriteDisLQDamage2Xls_gj(MSExcel.Worksheet _Worksheet,
        ...
        public static void Convent_Damage2023(string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval, string fileName)
        {
            ...
            foreach (string roadTypeInfo in roadTypeSplit)
            {
                ...
                WriteDis_Damage2Xls_gj2023(Destxls, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, i, startMileD, endMileStrD);
            }
        }

        private static void WriteDis_Damage2Xls_gj2023(string path,
  ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int roadType, double sMile, double eMile)
        {
            List<string> allDatas = new List<string>();
            if (roadType == 0)
            {
                allDatas.Add(GJTitles.LQ_2018_SMALL);
            }
            else
            {
                allDatas.Add(GJTitles.SN_2018_SMALL);
            }
            string errlog = prjdir.FullName + "\\errlog.txt";
...
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M)
        ...
        public static void Convent_Damage2024_ChongQing(string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval, string fileName, Encoding encoding)
        {
            ...
            foreach (string roadTypeInfo in roadTypeSplit)
            {
                ...
                string Destxls = string.Format(@"{0}\{1}.txt", path, tempName);
                WriteDis_Damage2Xls_gj2024_ChongQing(Destxls, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, i, startMileD, endMileStrD, encoding);
            }
        }

        public static void Convent_Damage2024(string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval, string fileName, Encoding encoding)
        {
            string[] roadTypeSplit = File.ReadAllLines(prjdir.FullName + "\\RoadTypeInfo.txt");
            ...
            foreach (string roadTypeInfo in roadTypeSplit)
            {
                ...
                string Destxls = string.Format(@"{0}\{1}.txt", path, tempName);
                WriteDis_Damage2Xls_gj2024(Destxls, prjinfo, prjdir, _RoadPart, _SmallRoadDisList, i, startMileD, endMileStrD, encoding);
            }
        }
        private static void WriteDis_Damage2Xls_gj2024_ChongQing(string path,
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int roadType, double sMile, double eMile, Encoding encoding)
        {
            List<string> allDatas = new List<string>();
            if (roadType == 0)
            {
                ...
                if (roadpart[i].roadtype == roadType)
                {
                    ...
                    for (int dis = 0; dis < RoadDiseaseTypes.roaddis[roadType].Length; dis++)
                    {
                        if (roadType == 0)
                        {
                            ...
                            {
                                line += ",";
                                double area = RoadDiseaseTypes.roaddis[roadType][dis].totalarea /0.2;
                            ...
                            {
                                line += ",";
                                double area = (RoadDiseaseTypes.roaddis[roadType][dis].totalarea + RoadDiseaseTypes.roaddis[roadType][dis + 1].totalarea)/0.4;
                                line +=  area.ToString("f2");
                                dis++;
                            }
                            else if (dis == 11)
                        ...
                        else
                        {
                            ...
                            {
                                line += ",";
                                double area = RoadDiseaseTypes.roaddis[roadType][dis].totalarea/1.0;
                                line +=  area.ToString("f2");
                            }
        ...
        private static void WriteDis_Damage2Xls_gj2024(string path,
 ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis, int roadType, double sMile, double eMile,Encoding encoding)
        {
            List<string> allDatas = new List<string>();
            if (roadType == 0)
        ...
        private static void writeAutoTestXls_Lbi2023(MSExcel.Worksheet worksheet_snhz,
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M)
        {
            ExcelGPS[] dicGps = null;
            ...
            destrange = worksheet_snhz.get_Range(String.Format("A2:D{0}", len + 1));
            destrange.Value2 = disvalsn;
            destrange = worksheet_snhz.get_Range(String.Format("A2:D{0}", rowcnt + 1));
            GlobalExcel.SetBorderLine(destrange, 53);
            _Setting.IsExcelSort = true;
            if (_Setting.IsExcelSort && prjinfo._Direction > 0)
            {
...
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M, double[] LRutVal, double[] RRutVal, double[] SRutVal, int lastCol)
        ...
        private static void writeAutoTestXls_RD2023(MSExcel.Worksheet worksheet_snhz,
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M, double[] LRutVal, double[] RRutVal, double[] SRutVal, int lastCol)
        {
            if (!prjinfo._IsRut)
                return;
        ...
        public static void Convent_Mpd2023(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, string fileName)
        {
            string excelFileName = "磨耗_MPD.xlsx";
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\国检数据格式\{1}",
                System.Windows.Forms.Application.StartupPath, excelFileName);
            ...
            MSExcel.Worksheet _Worksheet_ = _Workbook.Sheets["sheet1"] as MSExcel.Worksheet;
            writeAutoTestXls_Mpd2023(_Worksheet_, prjinfo, prjdir, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, 4);
        ...
        private static void writeAutoTestXls_Mpd2023(MSExcel.Worksheet worksheet_snhz,
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M, double[] LMTDVal, double[] RMTDVal, double[] CMTDVal, int lastCol)
        {
            if (!prjinfo._IsIRIMTD)
            {
        ...
        public static void Convent_Bump2023(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, string fileName)
        {
            string excelFileName = "跳车_PB.xlsx";
            string srcxls = string.Format(@"{0}\报表模板\等级公路 JTG H20-2018\国检数据格式\{1}",
                System.Windows.Forms.Application.StartupPath, excelFileName);
            ...
            MSExcel.Worksheet _Worksheet_ = _Workbook.Sheets["sheet1"] as MSExcel.Worksheet;
            writeAutoTestXls_Bump2023(_Worksheet_, prjinfo, prjdir, _RoadPart, _PBIVal, _LDeltaHVal, _RDeltaHVal, 5);
        ...
        private static void writeAutoTestXls_Bump2023(MSExcel.Worksheet worksheet_snhz,
ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> RoadPart10M, int[][] PBIVal, double[] LDeltaHVal, double[] RDeltaHVal, int lastCol)
        {
            if (!prjinfo._IsRut)
                return;
        ...
        #region 甘肃定制 20260104
        public static void OutputAllInfo(MSExcel.Workbook _Workbook, string outPath, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            MSExcel.Worksheet _Worksheet = null;
            string sheetName;
            string disValTxt = "十米";
            ...
            else if (disval == 1000)
            {
                ...
                _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
                //写入病害明细表格
                WriteAllDisLB2Xls(_Worksheet, prjinfo, prjdir, _SmallRoadDisList, _RoadPart);
            }
             sheetName = $"综合明细{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
      
            WriteAllPDMXXls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList,
               _LIRIMeanVal, _RIRIMeanVal, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal,
               _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, _PBIVal,_MarkVal);

            sheetName = $"沥青路面损坏{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            
            WriteAllLqDisHZ2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);
 
             
            sheetName = $"水泥路面损坏{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            WriteAllSnDisHZ2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _SmallRoadDisList);

            sheetName = $"平整度{disValTxt}"; 
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            WriteAllIRI2Xls(_Worksheet, prjinfo, _RoadPart, _LIRIMeanVal, _RIRIMeanVal, _SpeedVal, _MarkVal, disval);

            sheetName = $"车辙{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            WriteAllRut2Xls(_Worksheet, prjinfo, _RoadPart, _LRutMeanVal, _RRutMeanVal, _SRutMeanVal, _MarkVal,disval);

            sheetName = $"构造深度{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            WriteAllMtd2Xls(_Worksheet, prjinfo, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _MarkVal);
            sheetName = $"跳车{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet;
            WriteAllPB2Xls(_Worksheet, prjinfo, prjdir, _RoadPart, _LDeltaHVal, _RDeltaHVal, _PBIVal, _MarkVal, disval);

            sheetName = $"磨耗{disValTxt}";
            _Worksheet = _Workbook.Sheets[sheetName] as MSExcel.Worksheet; 
            WriteAllPWI2Xls(_Worksheet, prjinfo, _RoadPart, _LMTDMeanVal, _RMTDMeanVal, _CMTDMeanVal, _MarkVal);

        }
        private static void WriteAllDisLB2Xls (MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir, SmalRectDisease[] arrdis, List<MilePart> roadpart)
        {
            _Worksheet.Select();
            int len = roadpart.Count - 1, dlen = arrdis.Length;
            if (len < 1 || dlen < 1)
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                    ...
                    || (prjinfo._Direction < 0 && arrdis[j].m_mile <= smile && arrdis[j].m_mile > emile)))
                {
                    ...
                    {
                        string disName;
                        if (arrdis[j].RoadDisType == "修补.块状")
                        {
                            disName = "块修";
                        }
                        else if (arrdis[j].RoadDisType == "修补.条状")
                        {
                            disName = "条修";
                        }
                        else if (arrdis[j].RoadDisType.Contains("车辙"))
                        {
                            disName = "车辙";
                        }
                        else
                        {
                            disName = arrdis[j].RoadDisType;
                        }
                        string[] s = disName.Split('.');
                        vallist[rowcnt, 0] = j+1;
                        vallist[rowcnt, 1] = prjinfo._RoadCode;
                        vallist[rowcnt, 2] = prjinfo._Direction == 0 ? "上行" :"下行";  
                        vallist[rowcnt, 3] = arrdis[j].m_mile;
                        vallist[rowcnt, 4] =  prjinfo._Direction == 0 ?"A":"B";
                        vallist[rowcnt, 5] = GlobalExcel._RoadTypeStr[roadpart[i].roadtype] + "路面"; 
                        ...
                        vallist[rowcnt, 8] = arrdis[j].Area / RoadDiseaseTypes.roaddis[roadpart[i].roadtype][typeidx].weight;
                        vallist[rowcnt, 9] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][typeidx].weight;
                        vallist[rowcnt, 10] = arrdis[j].Area;
                     
                        vallist[rowcnt, 11] = arrdis[j].m_DistanceToRight;
                        vallist[rowcnt, 12] = "";
                        ++rowcnt;
                    }
                    else
            ...
            MSExcel.Range destrange = _Worksheet.get_Range(String.Format("A6:M{0}", dlen + 5));
            destrange.Value2 = vallist; 
        }

        private static void WriteAllPDMXXls(MSExcel.Worksheet worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir,
          List<MilePart> roadpart, SmalRectDisease[] arrdis,
          double[] LIRIVal, double[] RIRIVal, double[] LRutVal, double[] RRutVal, double[] SRutVal,
          double[] LMTDVal, double[] RMTDVal, double[] CMTDVal, int[][] PBVal, string[] MarkVal)
        {
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                vallist[rowcnt, 1] = smile;
                vallist[rowcnt, 2] = emile;
                vallist[rowcnt, 3] = prjinfo._Direction > 0 ? "A" : "B";
                vallist[rowcnt, 4] = prjinfo._Direction > 0 ? "上行" : "下行";
                vallist[rowcnt, 5] = prjinfo._RoadGrade ;
                vallist[rowcnt, 6] = GlobalExcel._RoadTypeStr[roadpart[i].roadtype]+ "路面";

                drval = ComputPCI(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                vallist[rowcnt, 9] = drval;

                //pcival = 100 - _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0] * Math.Pow(drval, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);
                //vallist[rowcnt, 10] = Math.Round(pcival, 5);

                vallist[i, 10] = string.Format("=IF(G{0}=\"沥青路面\",IF(100-{1}*POWER(J{0},{2})>0,100-{1}*POWER(J{0},{2}),100-{1}*POWER(J{0},{2})),IF(100-{3}*POWER(J{0},{4})>0,100-{3}*POWER(J{0},{4}),0))",
                  i + DataStartXlsxRow,
                  _PCIa[roadpart[i].roaddegree][0][0],
                  _PCIa[roadpart[i].roaddegree][0][1],
                  _PCIa[roadpart[i].roaddegree][1][0],
                ...
                #region  判断是否存在IRIMTD文件夹  
                 ...
                 //   trqival = 100 / (1 + _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0] * Math.Exp(_RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1] * irival));
                    vallist[rowcnt,12] = Math.Round(irival, 5); 

                vallist[i, 13] = String.Format("=ROUND(100/(1+{0}*EXP({1}*M{2})),5)",
                 _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1], i + DataStartXlsxRow);
                vallist[i, 14] = string.Format("=IF(N{0}>={1},\"优\",IF(N{0}>={2},\"良\",IF(N{0}>={3},\"中\",IF(N{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow,
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][1],
                    _RQIGrade[roadpart[i].roaddegree][roadpart[i].roadtype][2],
                ...
                if (prjinfo._IsIRIMTD && PBVal != null && PBVal.Length > 0)
                {
                    vallist[rowcnt, 19] = string.Format("=IF((100-{0}*{1}-{2}*{3}-{4}*{5})>0,(100-{0}*{1}-{2}*{3}-{4}*{5}),0)",
                        PBVal[i][1], _PBIScore[1],
                        PBVal[i][2], _PBIScore[2],
                        PBVal[i][3], _PBIScore[3]);
                    vallist[rowcnt, 20] = string.Format("=IF(T{0}>={1},\"优\",IF(T{0}>={2},\"良\",IF(T{0}>={3},\"中\",IF(T{0}>={4},\"次\",\"差\"))))",
                       i + DataStartXlsxRow,
                        _PBIGrade[roadpart[i].roaddegree][0],
                        _PBIGrade[roadpart[i].roaddegree][1],
                        _PBIGrade[roadpart[i].roaddegree][2],
                ...
                { 
                    ...
                    vallist[rowcnt, 21] = Math.Round(wrval, 5);
                    vallist[rowcnt, 22] = string.Format("=IF((100-{0}*POWER({1},{2}))>0,(100-{0}*POWER({1},{2})),0)", _PWIa[0], wrval, _PWIa[1]);
                    vallist[rowcnt, 23] = string.Format("=IF(W{0}>={1},\"优\",IF(W{0}>={2},\"良\",IF(W{0}>={3},\"中\",IF(W{0}>={4},\"次\",\"差\"))))",
                        i + DataStartXlsxRow,
                        _PWIGrade[roadpart[i].roaddegree][0],
                        _PWIGrade[roadpart[i].roaddegree][1],
                        _PWIGrade[roadpart[i].roaddegree][2],
                ...
                if (prjinfo._IsRut)
                {
                    ...
                    vallist[rowcnt, 15] = rutval;

                    vallist[i, 16] = string.Format("=IF(P{0}<{1},{2}-{3}*P{0},IF(P{0}<{4},{5}-{6}*(P{0}-{1}),0))",
                       i + DataStartXlsxRow, _RDIRD[0][1], _RDIRD[0][0], _RDIa[0], _RDIRD[1][1], _RDIRD[1][0], _RDIa[1]);
                    vallist[i, 17] = string.Format("=IF(G{0}>={1},\"优\",IF(G{0}>={2},\"良\",IF(G{0}>={3},\"中\",IF(G{0}>={4},\"次\",\"差\"))))",
                        i + DataStartXlsxRow, _RDIGrade[roadpart[i].roaddegree][0], _RDIGrade[roadpart[i].roaddegree][1], _RDIGrade[roadpart[i].roaddegree][2], _RDIGrade[roadpart[i].roaddegree][3]); 
                }
                else
                {
                ...
                //SMTD
                vallist[rowcnt, 18] = Math.Round((LMTDVal[i] + RMTDVal[i])/2,5);
                ...
                if (roadpart[i].roaddegree <= 1)
                {
                    if (_Setting.Is_SnCarve == 1 && roadpart[i].roadtype == 1)//有刻槽,并且是水泥，pwi不参与计算
                    {
                        vallist[rowcnt, 7] = string.Format("=IF(G{0}=\"沥青路面\",ROUND(({1}*K{0}+{2}*N{0}+{3}*IF(EXACT(Q{0},\"-\"),0,Q{0})+{4}*IF(EXACT(T{0},\"-\"),0,T{0})+{5}*IF(EXACT(W{0},\"-\"),0,W{0}))/({1}+{2}+{3}+{4}+{5}),5),ROUND(({6}*K{0}+{7}*N{0}+{8}*IF(EXACT(Q{0},\"-\"),0,Q{0})+{9}*IF(EXACT(T{0},\"-\"),0,T{0}))/({6}+{7}+{8}+{9}),5))",
                            i + DataStartXlsxRow,
                        _PQIW[roadpart[i].roaddegree][0][0],
                        _PQIW[roadpart[i].roaddegree][0][1],
                        _PQIW[roadpart[i].roaddegree][0][2],
                    ...
                    {
                        vallist[rowcnt, 7] = string.Format("=IF(G{0}=\"沥青路面\",ROUND(({1}*K{0}+{2}*N{0}+{3}*IF(EXACT(Q{0},\"-\"),0,Q{0})+{4}*IF(EXACT(T{0},\"-\"),0,T{0})+{5}*IF(EXACT(W{0},\"-\"),0,W{0}))/({1}+{2}+{3}+{4}+{5}),5),ROUND(({6}*K{0}+{7}*N{0}+{8}*IF(EXACT(Q{0},\"-\"),0,Q{0})+{9}*IF(EXACT(T{0},\"-\"),0,T{0})+{10}*IF(EXACT(W{0},\"-\"),0,W{0}))/({6}+{7}+{8}+{9}+{10}),5))",
                            i + DataStartXlsxRow,
                          _PQIW[roadpart[i].roaddegree][0][0],
                          _PQIW[roadpart[i].roaddegree][0][1],
                          _PQIW[roadpart[i].roaddegree][0][2],
                ...
                {
                    vallist[rowcnt, 7] = string.Format("=IF(G{0}=\"沥青路面\",ROUND(({1}*K{0}+{2}*N{0})/({1}+{2}),5),ROUND(({3}*K{0}+{4}*N{0})/({3}+{4}),5))",
                        i + DataStartXlsxRow,
                        _PQIW[roadpart[i].roaddegree][0][0],
                        _PQIW[roadpart[i].roaddegree][0][1],
                        _PQIW[roadpart[i].roaddegree][1][0],
                ...
                vallist[rowcnt,8] = string.Format("=IF(H{0}>={1},\"优\",IF(H{0}>={2},\"良\",IF(H{0}>={3},\"中\",IF(H{0}>={4},\"次\",\"差\"))))",
                    i + DataStartXlsxRow,
                  _PQIGrade[roadpart[i].roaddegree][0],
                  _PQIGrade[roadpart[i].roaddegree][1],
                  _PQIGrade[roadpart[i].roaddegree][2],
                  _PQIGrade[roadpart[i].roaddegree][3]); 
                vallist[rowcnt, 24] = MarkVal[i];
                ++rowcnt;
            }
            MSExcel.Range destrange = worksheet.get_Range(String.Format("A6:Y{0}", rowcnt + 5));
            destrange.Value2 = vallist;
        }


        private static void WriteAllLqDisHZ2Xls(  MSExcel.Worksheet worksheet_lqhz,
            ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis )
        {
            worksheet_lqhz.Select();
            MSExcel.Range destrange;
            int disnum = 0;
            object[,] disval;   
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                if (roadpart[i].roadtype == 0)//沥青
                {
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = prjinfo._RoadCode ;  //车道
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = smile;
                    worksheet_lqhz.Cells[rowcnt_lq_s, colcnt++] = emile;  
                    disnum = RoadDiseaseTypes.DiseaseTypeDict[roadpart[i].roadtype].Count;

                    disval = new object[1, disnum + 6];
                    for (int di = 0, kk = 0; di < disnum; ++di, ++colcnt, ++kk)
                    {
                         
                        if (di == 5)
                        {
                            disval[0, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][5].totalarea+
                                 RoadDiseaseTypes.roaddis[roadpart[i].roadtype][6].totalarea ;
                            di++;
                        }
                        if (di==6)
                        {
                    ...
                    drval = ComputPCI_AllDisArea(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                    disnum--;
                    disval[0, disnum] = drval;
                    disval[0, disnum + 1] = string.Format("=100-{0}*POWER(P{1},{2})",
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                    ...
                    destrange = worksheet_lqhz.get_Range(string.Format("D{0}:{1}{0}", rowcnt_lq_s, GlobalExcel.GetCol((char)('D' + disnum + 6))));
                    destrange.Value2 = disval; 
                    rowcnt_lq_s++;
                }  
            } 
        ...
        private static void WriteAllSnDisHZ2Xls(MSExcel.Worksheet worksheet_snhz,
           ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, SmalRectDisease[] arrdis)
        {
            worksheet_snhz.Select();
            MSExcel.Range destrange;
            int disnum = 0;
            object[,] disval;
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                //病害汇总表
                int colcnt = 1;
                if (roadpart[i].roadtype == 1)//沥青
                {
                    worksheet_snhz.Cells[rowcnt_sn_s, colcnt++] = prjinfo._RoadCode ;  //车道
                    worksheet_snhz.Cells[rowcnt_sn_s, colcnt++] = smile;
                    worksheet_snhz.Cells[rowcnt_sn_s, colcnt++] = emile;
                    disnum = RoadDiseaseTypes.DiseaseTypeDict[roadpart[i].roadtype].Count;
                    ...
                    destrange = worksheet_snhz.get_Range(string.Format("D{0}:{1}{0}", rowcnt_sn_s, GlobalExcel.GetCol((char)('D' + disnum + 5))));
                    destrange.Value2 = disval;
                    rowcnt_sn_s++;
                }
            }
        ...
        private static void WriteAllIRI2Xls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
            List<MilePart> roadpart, double[] LIRIVal, double[] RIRIVal, double[] SpeedVal, string[] MarkVal,int dis )
        {
            _Worksheet.Select();
            if (!prjinfo._IsIRIMTD)
            {
            ...
            if (dis !=10)
            {
                vallist = new object[len, 6];
            }
            for (int i = 0; i < len; i++)
            {
                int col = 0;
                vallist[i,col++ ]= prjinfo._RoadCode ;
                vallist[i,col++ ]= roadpart[i].mile;
                vallist[i, col++] = roadpart[i + 1].mile;

                if (dis == 10)
                {
                    if (prjinfo._IsDIRIMTD)
                    {
                        vallist[i, col++] = LIRIVal[i];
                        vallist[i, col++] = RIRIVal[i];
                    }
                    else
                    {
                        vallist[i, col++] = LIRIVal[i];
                    ...
                    if (_Setting.RQIJudgeType == 0)
                    {
                        vallist[i, col++] = String.Format("=ROUND(AVERAGE(D{0}:E{0}),5)", i + DataStartXlsxRow);
                    }
                    else if (_Setting.RQIJudgeType == 1)
                    {
                        vallist[i, col++] = String.Format("=ROUND(MAX(D{0}, E{0}),5)", i + DataStartXlsxRow);
                    }
                    vallist[i, col++] = String.Format("=ROUND(100/(1+{0}*EXP({1}*F{2})),5)",
                    _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1], i + DataStartXlsxRow);
                ...
                else
                {
                    if (prjinfo._IsDIRIMTD)
                    {
                        double iri = (LIRIVal[i] + RIRIVal[i]) / 2;
                        if (_Setting.RQIJudgeType == 0)
                        {
                            iri = (LIRIVal[i] + RIRIVal[i]) / 2;
                        }
                        else if (_Setting.RQIJudgeType == 1)
                         {
                          iri = Math.Max(LIRIVal[i], RIRIVal[i]);
                         }
                    ...
                    vallist[i, col++] = String.Format("=ROUND(100/(1+{0}*EXP({1}*D{2})),5)",
                    _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1], i + DataStartXlsxRow);

                } 
                vallist[i, col++] = MarkVal[i];
            }
            if (dis == 10)
            {
                destrange = _Worksheet.get_Range(String.Format("A{0}:H{1}", DataStartXlsxRow, len + DataStartXlsxRow - 1));
        ...
        private static void WriteAllMtd2Xls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
           List<MilePart> roadpart, double[] LMTDVal, double[] RMTDVal,  string[] MarkVal)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                vallist[i, 6] = MarkVal[i];
            }

            destrange = _Worksheet.get_Range(String.Format("A{0}:G{1}", DataStartXlsxRow, len + DataStartXlsxRow - 1));
            destrange.Value2 = vallist;

        }
        private static void WriteAllRut2Xls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
           List<MilePart> roadpart, double[] lRutVal,double[] rRutVal, double[] sRutVal,  string[] MarkVal,int dis)
        {
            _Worksheet.Select();
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                if (dis==10)
                {
                    vallist[i, col++] = lRutVal[i];
                    vallist[i, col++] = rRutVal[i];
                    vallist[i, col++] = sRutVal[i];
                    vallist[i, col++] = string.Format("=IF(F{0}<{1},{2}-{3}*F{0},IF(F{0}<{4},{5}-{6}*(F{0}-{1}),0))",
                        i + DataStartXlsxRow, _RDIRD[0][1], _RDIRD[0][0], _RDIa[0], _RDIRD[1][1], _RDIRD[1][0], _RDIa[1]);
                ...
                {
                    vallist[i, col++] = sRutVal[i];
                    vallist[i, col++] = string.Format("=IF(D{0}<{1},{2}-{3}*D{0},IF(D{0}<{4},{5}-{6}*(D{0}-{1}),0))",
                        i + DataStartXlsxRow, _RDIRD[0][1], _RDIRD[0][0], _RDIa[0], _RDIRD[1][1], _RDIRD[1][0], _RDIa[1]);
                }
        ...
        private static void WriteAllPWI2Xls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo,
           List<MilePart> roadpart, double[] LMTDVal, double[] RMTDVal, double[] CMTDVal, string[] MarkVal )
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                {
                    vallist[i, 6] = string.Format("=IF(E{0}-MIN(D{0},F{0})>0, 100*(E{0}-MIN(D{0},F{0}))/E{0},0) ", i + DataStartXlsxRow);
                }
                vallist[i, 7] = string.Format("=100-{0}*POWER(G{1},{2})", _PWIa[0], i + DataStartXlsxRow, _PWIa[1]);
        ...
        private static void WriteAllPB2Xls(MSExcel.Worksheet _Worksheet, ProjectInfo prjinfo, DirectoryInfo prjdir,
             List<MilePart> roadpart, double[] LDeltaHVal, double[] RDeltaHVal, int[][] PBIVal, string[] MarkVal,int dis
           )
        {
            _Worksheet.Select();
            int DataStartXlsxRow = 6;
            ...
            if (dis == 10)
            {
                vallist = new object[len, 9];
            }
            
            for (i = 0; i < len; ++i)
            {
                int cnt = 0;
                vallist[i, cnt++] = prjinfo._RoadCode ;
                vallist[i, cnt++] = roadpart[i].mile;
                vallist[i, cnt++] = roadpart[i + 1].mile;

                vallist[i, cnt++] = PBIVal[i][1];
                vallist[i,cnt++] = PBIVal[i][2];
                vallist[i,cnt++] = PBIVal[i][3];
                if (dis == 10)
                {
                    double lDeltaH = 0;
                    double rDeltaH = 0;
                    if (i < LDeltaHVal.Length)
                    {
                        lDeltaH = LDeltaHVal[i];
                    }
                    if (prjinfo._IsDIRIMTD)
                    {
                        if (i < RDeltaHVal.Length)
                        {
                            rDeltaH = RDeltaHVal[i];
                        }
                    }  
                    vallist[i, cnt++] = Math.Max(lDeltaH,rDeltaH);
                } 
                vallist[i, cnt++] = string.Format("=IF((100-D{0}*{1}-E{0}*{2}-F{0}*{3})>0,(100- D{0}*{1}-E{0}*{2}-F{0}*{3}),0)",
                   i + DataStartXlsxRow, _PBIScore[1], _PBIScore[2], _PBIScore[3]);
                vallist[i, cnt++] = MarkVal[i]; 
