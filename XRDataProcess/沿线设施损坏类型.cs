...
{
    public partial class 沿线设施损坏类型 : Form
    {
        ...
        public 沿线设施损坏类型(int mile, int dir, int smile, int emile, int side = -1, Rectangle  rect = default)
        {
            InitializeComponent();
        ...
        private void AddDiseaseControls(TableLayoutPanel panel)
        {
            int i = 0;
            int rcnt = 1;
            foreach (StreetDiseaseType dis in DiseaseTypes.streetdislist)
            {
        ...
        void dismile_TextChanged(object sender, EventArgs e)
        {
            ...
            if (DiseaseTypes.streetdislist[rcnt - 1].unitval == 0)
            {
        ...
        double ComputeScore(int idx, double dislen, double discnt)
        {
            double score = 0;
           string desc =   DiseaseTypes.streetdislist[idx].description;
            if (DiseaseTypes.streetdislist[idx].unitval == 0)
            {
                if (_Setting.ParmStyle== StandardParmType.RuralRoadlowLevel)
                {
                    score = dislen * DiseaseTypes.streetdislist[idx].unitscore;
                }
                else
                {
                    score = (int)Math.Ceiling(dislen * DiseaseTypes.streetdislist[idx].unitscore);

                }
            }
            else
            { 
                    score = (int)((int)Math.Ceiling(discnt) * DiseaseTypes.streetdislist[idx].unitscore); 
            }
            return score;
        }
        ...
        {
            for (int i = 0; i < DiseaseTypes.streetdislist.Count; ++i)
            {
                double score = Convert.ToDouble(tableLayoutPanel1.GetControlFromPosition(3, i + 1).Text);
                if (score > 0)
                {
                    ...
                    _DisRecord.Add(new StreetDisRecord(mile, DiseaseTypes.streetdislist[i].disname, score, num, len,_side,_rect));
                }
            }
