...
namespace XRDataProcess
{
    ...
    class MyExcelLNDegree
    {
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\路面平整度评价等级记录表.xlsx",
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\路面病害面积统计表.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, Disease[] arrdis)
        {
            ...
            for (int i = 0, j = 0; i < len - 1; i++)//i区间索引，j病害索引
            {
                ...
                if (roadpart[i].roadtype == 1)//水泥
                {
                    ...
                    disval[0, disnum + 1] = string.Format("=100-{0}*POWER(O{1},{2})",
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\路面破损评价等级记录表.xlsx",
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\路面综合评价等级记录表.xlsx",
                System.Windows.Forms.Application.StartupPath, disval);
        ...
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\CPMS路面损坏调查表.xlsx",
                System.Windows.Forms.Application.StartupPath);
        ...
        //奥路通模板
        public static void OutputALT(MSExcel.Application excelApp, string path, ProjectInfo prjinfo, DirectoryInfo prjdir, int disval)
        {
            string srcxls = string.Format(@"{0}\报表模板\辽宁农村公路\奥路通模板.xlsx",
                System.Windows.Forms.Application.StartupPath);
            ...
            MSExcel.Worksheet _Worksheet_mx = _Workbook.Sheets["路面技术状况明细表"] as MSExcel.Worksheet;

            WriteALT2Xls(_Worksheet_lqhz, _Worksheet_snhz, _Worksheet_lqtj, _Worksheet_sntj,
                _Worksheet_mx, prjinfo, prjdir, _RoadPart, _RoadDisList, _LIRIMeanVal, _RIRIMeanVal, disval);
        ...
        private static void WriteALT2Xls(MSExcel.Worksheet worksheet_lqhz, MSExcel.Worksheet worksheet_snhz,
            MSExcel.Worksheet worksheet_lqtj, MSExcel.Worksheet worksheet_sntj, MSExcel.Worksheet worksheet_mx, 
            ProjectInfo prjinfo, DirectoryInfo prjdir, List<MilePart> roadpart, Disease[] arrdis, double[] LIRIVal, double[] RIRIVal, int partlenval)
        {
            MSExcel.Range destrange;
            int disnum = 0;
            object[,] disval;
            ...
            for (int i = 0, j = 0; i < len; i++)//i区间索引，j病害索引
            {
                ...
                int emile = roadpart[i + 1].mile;
                int milelength = Math.Abs(smile - emile);
                string milepartstr = string.Format("K{0:0000}+{1:000}-K{2:0000}+{3:000}", smile / 1000, smile % 1000, emile / 1000, emile % 1000);
                ...
                if (roadpart[i].roadtype == 1)//水泥
                {
                    ...
                    drval = ComputPCI(RoadDiseaseTypes.roaddis, roadpart[i].roadtype, _RoadConfig.DetectWidth * milelength);
                    disval[0, 0] = milepartstr;
                    disval[0, 1] = 100 - _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0] * Math.Pow(drval, _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]);
                    for (int di = 0, kk = 2; di < disnum; ++di, ++colcnt, ++kk)
                    {
                        disval[0, kk] = RoadDiseaseTypes.roaddis[roadpart[i].roadtype][di].totalarea;
                        if (kk == disnum - 1)
                        {
                    ...
                    destrange = worksheet_snhz.get_Range(string.Format("A{0}:{1}{0}", rowcnt_sn_s, GlobalExcel.GetCol((char)('A' + disnum + 1))));
                    destrange.Value2 = disval;
                ...
                else if (roadpart[i].roadtype == 0)//沥青
                {
                    ...
                    destrange = worksheet_lqhz.get_Range(string.Format("A{0}:{1}{0}", rowcnt_lq_s, GlobalExcel.GetCol((char)('A' + disnum + 1))));
                    destrange.Value2 = disval;
                ...
                //明细汇总表
                vallist[rowcnt, 0] = milepartstr;
                vallist[rowcnt, 1] = milelength;
                vallist[rowcnt, 2] = drval;

                if (prjinfo._IsDIRIMTD)
                    vallist[rowcnt, 3] = (LIRIVal[i] + RIRIVal[i]) / 2;
                else
                    vallist[rowcnt, 3] = LIRIVal[i];

                vallist[rowcnt, 4] = string.Format("=100-{1}*POWER(C{0},{2})", rowcnt + 4,
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        _PCIa[roadpart[i].roaddegree][roadpart[i].roadtype][1]); 
                
                vallist[rowcnt, 5] = String.Format("=100/(1+{0}*EXP({1}*D{2}))",
                    _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][0], _RQIa[roadpart[i].roaddegree][roadpart[i].roadtype][1], rowcnt + 4);
                
                vallist[rowcnt, 6] = string.Format("=({1}*E{0}+{2}*F{0})/({1}+{2})",
                        rowcnt + 4,
                        _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][0],
                        _PQIW[roadpart[i].roaddegree][roadpart[i].roadtype][1]);

                vallist[rowcnt, 7] = _RoadGradeStr[roadpart[i].roaddegree];

                vallist[rowcnt, 8] = string.Format("=IF(F{0}>={1},\"优\",IF(F{0}>={2},\"良\",IF(F{0}>={3},\"中\",IF(F{0}>={4},\"次\",\"差\"))))",
                    rowcnt + 4,
                    _RQIGrade[roadpart[i].roaddegree][0],
                    _RQIGrade[roadpart[i].roaddegree][1],
                    _RQIGrade[roadpart[i].roaddegree][2],
                ...
                vallist[rowcnt, 9] = string.Format("=IF(E{0}>={1},\"优\",IF(E{0}>={2},\"良\",IF(E{0}>={3},\"中\",IF(E{0}>={4},\"次\",\"差\"))))",
                    rowcnt + 4,
                    _PCIGrade[roadpart[i].roaddegree][0],
                    _PCIGrade[roadpart[i].roaddegree][1],
                    _PCIGrade[roadpart[i].roaddegree][2],
                ...
                vallist[rowcnt, 10] = string.Format("=IF(G{0}>={1},\"优\",IF(G{0}>={2},\"良\",IF(G{0}>={3},\"中\",IF(G{0}>={4},\"次\",\"差\"))))",
                    rowcnt + 4,
                    _PQIGrade[roadpart[i].roaddegree][0],
                    _PQIGrade[roadpart[i].roaddegree][1],
                    _PQIGrade[roadpart[i].roaddegree][2],
            ...
            for (int di = 0; di < disnum; di++)
            {
                disval[0, di] = string.Format("=SUM({0}4:{0}{1})/2", GlobalExcel.GetCol((char)('C' + di)), rowcnt_sn_s - 1);
            }
            destrange = worksheet_snhz.get_Range(string.Format("C{0}:{1}{0}", rowcnt_sn_s, GlobalExcel.GetCol((char)('C' + disnum - 1))));
            destrange.Value2 = disval;
            destrange = worksheet_snhz.get_Range(String.Format("A1:{0}{1}", GlobalExcel.GetCol((char)('C' + disnum - 1)), rowcnt_sn_s));
            GlobalExcel.SetBorderLine(destrange, 21);
            ...
            for (int di = 0; di < disnum; di++)
            {
                disval[0, di] = string.Format("=SUM({0}4:{0}{1})/2", GlobalExcel.GetCol((char)('C' + di)), rowcnt_lq_s - 1);
            }
            destrange = worksheet_lqhz.get_Range(string.Format("C{0}:{1}{0}", rowcnt_lq_s, GlobalExcel.GetCol((char)('C' + disnum - 1))));
            destrange.Value2 = disval;
            destrange = worksheet_lqhz.get_Range(String.Format("A1:{0}{1}", GlobalExcel.GetCol((char)('C' + disnum - 1)), rowcnt_lq_s));
            GlobalExcel.SetBorderLine(destrange, 21);
            ...
            if (Haslqflag)
            {
                ...
                for (int i = 0; i < disnum; i++)
                {
                    disval[i, 0] = string.Format("=SUMIF(沥青路面病害汇总表!{0}:{0},\"<>\",沥青路面病害汇总表!{0}:{0})/3", Convert.ToChar('C' + i));
                }
                destrange = worksheet_lqtj.get_Range("B4:B" + (disnum + 3).ToString());
                destrange.Value2 = disval;
            }
            ...
            if (Hassnflag)
            {
                ...
                for (int i = 0; i < disnum; i++)
                {
                    disval[i, 0] = string.Format("=SUMIF(水泥路面病害汇总表!{0}:{0},\"<>\",水泥路面病害汇总表!{0}:{0})/3", Convert.ToChar('C' + i));
                }
                destrange = worksheet_sntj.get_Range("B4:B" + (disnum + 3).ToString());
                destrange.Value2 = disval;
            }
            ...
            worksheet_mx.Cells[2, 2] = prjinfo._RoadName;
            worksheet_mx.Cells[2, 4] = prjinfo._RoadCode;
            worksheet_mx.Cells[2, 10] = prjinfo._RoadNum;
            destrange = worksheet_mx.get_Range(String.Format("A4:K{0}", rowcnt + 3));
            destrange.Value2 = vallist;
            destrange = worksheet_mx.get_Range(String.Format("A1:K{0}", rowcnt + 3));
            GlobalExcel.SetBorderLine(destrange, 21);
            
            MSExcel.ChartObject chartobj = null;
            MSExcel.Chart chart = null;
            destrange = worksheet_mx.get_Range(string.Format("A3:A{0},E3:G{0}", rowcnt + 3));
            chartobj = (MSExcel.ChartObject)worksheet_mx.ChartObjects(1);
            chart = chartobj.Chart;
            chart.ChartWizard(destrange, MSExcel.XlChartType.xlLine, 2, MSExcel.XlRowCol.xlColumns, 1, 1, true, Type.Missing, Type.Missing, Type.Missing, Type.Missing);

            destrange = worksheet_mx.get_Range(string.Format("A3:A{0},C3:C{0}", rowcnt + 3));
            chartobj = (MSExcel.ChartObject)worksheet_mx.ChartObjects(2);
            chart = chartobj.Chart;
            chart.ChartWizard(destrange, MSExcel.XlChartType.xlLine, 2, MSExcel.XlRowCol.xlColumns, 1, 1, true, Type.Missing, Type.Missing, Type.Missing, Type.Missing);

            destrange = worksheet_mx.get_Range(string.Format("A3:A{0},D3:D{0}", rowcnt + 3));
            chartobj = (MSExcel.ChartObject)worksheet_mx.ChartObjects(3);
            chart = chartobj.Chart;
            chart.ChartWizard(destrange, MSExcel.XlChartType.xlLine, 2, MSExcel.XlRowCol.xlColumns, 1, 1, true, Type.Missing, Type.Missing, Type.Missing, Type.Missing);

            worksheet_mx.Name = worksheet_mx.Name + "（" + partlenval + "米）";
        }
    }
}
